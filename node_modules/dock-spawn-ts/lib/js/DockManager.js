...
export class DockManager {
  constructor(element, config) {
    if (element === undefined) throw new Error('Invalid Dock Manager element provided');
  ...
  initialize() {
    ...
    if (this.backgroundContext != null) {
      this.context.model.rootNode.container.tabHost.hostElement.insertBefore(this.backgroundContext, this.context.model.rootNode.container.tabHost.hostElement.firstChild);
    }
  ...
  checkXBounds(container, currentMousePosition, previousMousePosition, resizeWest, resizeEast) {
    if (this._config.moveOnlyWithinDockConatiner) return this.checkXBoundsWithinDockContainer(container, currentMousePosition, previousMousePosition, resizeWest, resizeEast);
  ...
  checkYBounds(container, currentMousePosition, previousMousePosition, resizeNorth, resizeSouth) {
    if (this._config.moveOnlyWithinDockConatiner) return this.checkYBoundsWithinDockContainer(container, currentMousePosition, previousMousePosition, resizeNorth, resizeSouth);
  ...
  setModel(model) {
    ...
    this.loadResize(model.rootNode); // this.invalidate();
  }
  ...
  loadResize(node) {
    ...
    node.container.resize(container.state.width, container.state.height); // node.performLayout();
  }
  ...
  _onDialogDragStarted(sender, e) {
    ...
    if (sender.noDocking == null || sender.noDocking !== true) this.dockWheel.showWheel();
  ...
  _onDialogDragEnded(sender, e) {
    ...
    delete this.dockWheel.activeDialog; //TODO: not so good
  ...
  floatDialog(container, x, y) {
    let retdiag = undefined; //check the dialog do not already exist
    ...
    if (retdiag) return retdiag; //try to undock just in case
  ...
  _requestDockDialog(referenceNode, dialog, layoutDockFunction) {
    ...
    dialog.destroy();
    layoutDockFunction(referenceNode, newNode); // this.invalidate();
  ...
  _requestDockContainer(referenceNode, container, layoutDockFunction, dockedToPrevious, ratio) {
    ...
    if (referenceNode.parent && referenceNode.parent.container) {
      ...
      if (oldSplitter.getRatios) ratios = oldSplitter.getRatios();
    }
    ...
    if (ratio && newNode.parent && (newNode.parent.container.containerType === 'vertical' || newNode.parent.container.containerType === 'horizontal')) {
      ...
      } else splitter.setContainerRatio(container, ratio);
    }
  ...
  requestUndockToDialog(container, event, dragOffset) {
    ...
    panelContainer.elementPanel.style.display = 'block'; // Create a new dialog window for the undocked panel
    ...
    if (event !== undefined) {
      ...
      if (dragOffset.x > dialogWidth) dragOffset.x = 0.75 * dialogWidth;
  ...
  */
  ...
  requestClose(container) {
    ...
    if (this.activePanel == container) this.activePanel = null;
    if (this._activeDocument == container) this._activeDocument = null;
  }
  /**
  ...
  requestRemove(container) {
    ...
    node.detachFromParent();
    if (parent) this.rebuildLayout(parent);
    return node;
  }
  ...
  getNodeByElementId(id) {
    ...
    while (stack.length > 0) {
      let topNode = stack.pop();
      if (topNode.container instanceof PanelContainer && topNode.container.elementContent.id === id) return topNode;
      [].push.apply(stack, topNode.children);
    }
  ...
  _findNodeFromContainer(container) {
    ...
    while (stack.length > 0) {
      let topNode = stack.pop();
      if (topNode.container === container) return topNode;
      [].push.apply(stack, topNode.children);
    }
  ...
  findNodeFromContainerElement(containerElement) {
    ...
    while (stack.length > 0) {
      let topNode = stack.pop();
      if (topNode.container.containerElement === containerElement) return topNode;
      [].push.apply(stack, topNode.children);
    }
  ...
  notifyOnClosePanel(panel) {
    ...
    if (this._activeDocument == panel) this._activeDocument = null;
  }
  ...
  getPanels() {
    let panels = []; //all visible nodes

    this._allPanels(this.context.model.rootNode, panels); //all visible or not dialogs
  ...
  updatePanels(ids) {
    ...
    this._allPanels(this.context.model.rootNode, panels); //only remove
  ...
  getVisiblePanels() {
    ...
    this._allPanels(this.context.model.rootNode, panels); //all visible
  ...
  set activePanel(value) {
    if (value !== this._activePanel) {
      ...
      if (this._lastPanelNotADialog && this.getPanels().indexOf(this._lastPanelNotADialog) < 0) this._lastPanelNotADialog = null;
