...
export class DockWheel {
  constructor(dockManager) {
    ...
    for (let wheelType in WheelTypes) {
      ...
      if (wheelType.substr(-2, 2) === '-s') // Side button
        this.elementSideWheel.appendChild(this.wheelItems[wheelType].element);else // Main dock wheel button
  ...
  set activeNode(value) {
    ...
    if (previousValue !== this._activeNode) {
      ...
      if (this._visible) this.showWheel();
    }
  }
  ...
  showWheel() {
    ...
    this.elementMainWheel.style.top = bcElement.top - bcdockManagerElement.top + baseY + 'px'; // The positioning of the main dock wheel buttons is done automatically through CSS
  ...
  _setWheelButtonPosition(wheelId, left, top) {
    ...
    let y = Math.floor(top - itemHalfHeight); //    item.element.style.left = '${x}px';
  ...
  hideWheel() {
    ...
    Utils.removeNode(this.elementMainWheel);
    Utils.removeNode(this.elementSideWheel);
    Utils.removeNode(this.elementPanelPreview); // deactivate all wheels

    for (let wheelType in this.wheelItems) this.wheelItems[wheelType].active = false;
  }

  onMouseOver(wheelItem) {
    if (!this.activeDialog) return; // Display the preview panel to show where the panel would be docked
  ...
  onDialogDropped(dialog) {
    ...
    if (wheelItem) this._handleDockRequest(wheelItem, dialog);
  }
  /**
  ...
  _getActiveWheelItem() {
    for (let wheelType in this.wheelItems) {
      ...
      if (wheelItem.active) return wheelItem;
    }
