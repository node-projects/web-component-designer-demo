...
import getYear from "../../date-fns/esm/getYear/index.js"; // import en from 'date-fns/esm/locale/en';
//
// const locales = { en, fr };
  ...
  localeChanged(locale) {
    if (!this.month) {
      switch (this.defaultAs) {
        case 'dateFrom':
          this.month = this.dateFrom ? `0${getMonth(parse(this.dateFrom, 't', new Date())) + 1}`.slice(-2) : format(new Date(), 'MM', {
            awareOfUnicodeTokens: true
          });
          break;

        case 'dateTo':
          this.month = this.dateTo ? `0${getMonth(parse(this.dateTo, 't', new Date())) + 1}`.slice(-2) : format(new Date(), 'MM', {
            awareOfUnicodeTokens: true
          });
          break;
