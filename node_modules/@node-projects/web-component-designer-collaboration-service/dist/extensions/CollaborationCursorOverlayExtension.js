import { AbstractExtension, OverlayLayer } from "@node-projects/web-component-designer";
function truncateText(text, maxLength) {
    if (!text || text.length <= maxLength)
        return text;
    return text.substring(0, Math.max(0, maxLength - 1)) + '…';
}
function createCursorPath(x, y) {
    return [
        `M ${x} ${y}`,
        `L ${x} ${y + 18}`,
        `L ${x + 4.5} ${y + 13.5}`,
...
export class CollaborationCursorOverlayExtension extends AbstractExtension {
    ...
    drawPeerCursor(peer) {
        ...
        this._drawTextWithBackground(truncateText(peer.displayName ?? peer.peerId, 18), x + 16, y + 16, color, 'svg-collaboration-cursor-label', undefined, OverlayLayer.Foreground);
    }
}
