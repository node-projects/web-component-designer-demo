export const defaultWebRtcTabCollaborationSignalingChannels = ['broadcast-channel', 'manual'];
function createRandomId() {
    return globalThis.crypto?.randomUUID?.() ?? `wcd-collab-${Date.now()}-${Math.floor(Math.random() * 100000)}`;
}
function createEnabledSignalingChannels(channels) {
    if (channels == null)
        return new Set(defaultWebRtcTabCollaborationSignalingChannels);
    const validChannels = channels.filter((channel) => channel === 'broadcast-channel' || channel === 'manual');
...
function cloneSignalingMessage(message) {
    return {
        ...message,
...
function normalizeJsonControlCharactersInStrings(data) {
    let normalized = '';
    let inString = false;
    let isEscaped = false;
...
function parseManualSignalingBundleData(data) {
    try {
...
export class WebRtcTabCollaborationTransport {
    _service = null;
    _session = null;
    _broadcastSignalingChannel = null;
    _enabledSignalingChannels = createEnabledSignalingChannels();
    _rtcConfiguration;
    _manualSignalingMessages = [];
    _peerConnections = new Map();
    _dataChannels = new Map();
    _initiatedPeers = new Set();
    _peersNeedingInitialSnapshot = new Set();
    _pendingCandidates = new Map();
    _processedSignalIds = new Set();
    _remoteConnectionIds = new Map();
    ...
    rememberRemoteConnection(remotePeerId, connectionId) {
    ...
    shouldRequestInitialSnapshot() {
        const snapshot = this._service?.createSnapshot();
        return !snapshot?.html?.trim();
    }
    updateInitialSnapshotRequest(remotePeerId, requestInitialSnapshot) {
    ...
    async acceptOffer(remotePeerId, description) {
        const peerConnection = this.getOrCreatePeerConnection(remotePeerId);
    ...
    async acceptAnswer(remotePeerId, description) {
        const peerConnection = this.getOrCreatePeerConnection(remotePeerId);
        await peerConnection.setRemoteDescription(description);
        await this.flushPendingCandidates(remotePeerId);
    }
    async acceptIceCandidate(remotePeerId, candidate) {
        const peerConnection = this.getOrCreatePeerConnection(remotePeerId);
    ...
    async flushPendingCandidates(remotePeerId) {
    ...
    async handleOpenChannel(remotePeerId) {
    ...
    async handleDataMessage(remotePeerId, rawData) {
        if (!this._service)
            return;
    ...
    cleanupPeer(remotePeerId, clearInitialSnapshotState = true) {
