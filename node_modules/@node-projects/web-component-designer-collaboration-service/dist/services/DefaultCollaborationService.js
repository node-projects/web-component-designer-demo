...
import { DomConverter, getCollaborationNodeIndex, getCollaborationNodeIndexes, getDesignItemByCollaborationNodeIndex } from '@node-projects/web-component-designer';
...
function createSyntheticTransactionItem(title) {
    return {
...
export class DefaultCollaborationService {
    _designerCanvas;
    _state = 'disconnected';
    _session = null;
    _transport = null;
    ...
    _lastLocalCursorPosition;
    _lastLocalCursorUpdate = 0;
    _refreshExtensionsHandle = null;
    constructor(_designerCanvas) {
        this._designerCanvas = _designerCanvas;
    ...
    connect(sessionId, peerId, displayName) {
    ...
    async applyRemoteChange(change, snapshot) {
        if (snapshot)
    ...
    updateRemoteSelection(peerId, selectedNodeIndexes, primaryNodeIndex) {
    ...
    updatePeerPresence(peer, source = 'local') {
    ...
    removeComment(commentId, source = 'local') {
    ...
    onStateChanged = new TypedEvent();
    onChange = new TypedEvent();
    onSelectionChanged = new TypedEvent();
    onPeersChanged = new TypedEvent();
    onCommentsChanged = new TypedEvent();
    storePeerPresence(peer, source) {
