...
function parseFontFamilies(family) {
    const seen = new Set();
    const families = [];
...
function normalizeCustomFontWeight(weight) {
...
function normalizeCustomFontStyle(style) {
...
async function convertFontToTrueType(data, format) {
    if (format === "ttf")
        return data;
...
export class PDFWriter {
    ...
    customFonts = new Map();
    customFontIndex = new Map();
    customFontIdCounter = 0;
    ...
    // ── Font helpers ────────────────────────────────────────────────
    async ensureFontAssetsRegistered() {
        if (this.fontAssetsRegistered || !this.fontAssets || this.fontAssets.faces.length === 0)
            return;
        for (const face of this.fontAssets.faces) {
            const source = choosePdfFontSource(face);
            if (!source)
                continue;
            if (source.format !== "ttf" && !this.useFontEditorCore)
                continue;
            try {
    ...
    registerCustomFont(family, data, descriptors) {
    ...
    addCustomFontCandidate(key, record) {
    ...
    getCustomFontForFamily(family, weight, style) {
    ...
    getCustomFontCandidates(family, weight, style) {
        const seen = new Set();
        const candidates = [];
        for (const record of [
    ...
    mapToPdfFont(family, weight, style) {
     ...
     */
    resolveFont(family, weight, style, text) {
    ...
    getCustomFontRecord(pdfFontName) {
    ...
    getCustomFontPdfName(pdfFontName) {
        const record = this.getCustomFontRecord(pdfFontName);
        if (record)
            return record.pdfName;
    ...
    async end() {
        ...
        for (const [pdfFontName, resName] of this.fontMap) {
            if (this.isCustomFont(pdfFontName)) {
                ...
                const usedChars = this.customFontUsedChars.get(pdfFontName.slice(7)) ?? new Set();
                if (parsed.isSymbolFont) {
     ...
     */
    createSimpleTrueTypeFont(doc, parsed, usedChars, pdfName) {
     ...
     */
    createCIDFont(doc, parsed, usedChars, pdfName) {
        const psName = pdfName;
        const scale = 1000 / parsed.unitsPerEm;
     ...
     */
    computeLinearGradientCoords(pdfPoints, angleDeg, cx, cy) {
        ...
        if (uxLen <= 0 || uyLen <= 0) {
            ...
            const halfDiag = Math.hypot(Math.max(...pdfPoints.map((point) => point.x)) - Math.min(...pdfPoints.map((point) => point.x)), Math.max(...pdfPoints.map((point) => point.y)) - Math.min(...pdfPoints.map((point) => point.y))) / 2;
    ...
    getUniformGradientOpacity(stops) {
        if (stops.length === 0)
            return 1;
    ...
    prepareGradientStopsForPdf(stops) {
        if (stops.length < 2)
