...
export function parseVisibleCssColor(color) {
    const parsed = parseCssColor(color);
    return parsed && parsed.a > 0 ? parsed : null;
}
export function getVisibleCssColorString(color) {
...
export function cssColorToTrueColor(color) {
    const parsed = parseVisibleCssColor(color);
    if (!parsed)
        return undefined;
    return (parsed.r << 16) | (parsed.g << 8) | parsed.b;
}
export function cssColorToColorRef(color) {
    const parsed = parseVisibleCssColor(color);
    if (!parsed)
        return null;
    return parsed.r | (parsed.g << 8) | (parsed.b << 16);
}
function parseCssColorUncached(color) {
...
function parseColorFunction(color) {
...
function parseLabFunction(color) {
    ...
    if (l === null || Number.isNaN(a) || Number.isNaN(b))
        return null;
    return fromLab(l, a, b, alpha);
}
function parseLchFunction(color) {
...
function parseOklchFunction(color) {
...
function parseUnitInterval(token) {
    if (token.endsWith("%")) {
        const value = parseFloat(token);
        if (Number.isNaN(value))
            return null;
...
function parseLightness(token, percentScale) {
    if (token.endsWith("%")) {
        const value = parseFloat(token);
        if (Number.isNaN(value))
            return null;
...
function fromLinearSrgb(r, g, b, alpha) {
    return {
