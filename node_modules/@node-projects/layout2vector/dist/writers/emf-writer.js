...
function getPageScaleHundredthMmPerPixel(width, height) {
...
class EmfBuffer {
    ...
    getNumRecords() { return this.numRecords; }
...
export class EMFWriter {
    ...
    async drawPolygon(points, style) {
        ...
        if (this.hasMixedBorders(style)) {
            ...
            if (fillColor !== null) {
                ...
                const ptData = int16Array(...points.flatMap(p => [Math.round(p.x), Math.round(p.y)]));
                this.records.writeRecord(EMR.POLYGON16, concat(int32Array(bounds.left, bounds.top, bounds.right, bounds.bottom), uint32Array(4), ptData));
        ...
        if (fillColor === null && !stroke && !outline)
            return;
        this.saveState();
        if (outline)
        ...
        if (radius > 0 && isAxisAlignedRect(points) && !style.cornerShapes) {
            ...
            if (fillColor !== null || stroke) {
                ...
                this.selectObject(brushHandle);
                this.selectObject(penHandle);
                this.records.writeRecord(EMR.ROUNDRECT, int32Array(Math.round(x), Math.round(y), Math.round(x + w), Math.round(y + h), Math.round(r * 2), Math.round(r * 2)));
                this.deleteObject(brushHandle);
                this.deleteObject(penHandle);
            }
            this.drawInsetBoxShadows(points, style);
        ...
        if (radius > 0 && (!isAxisAlignedRect(points) || style.cornerShapes)) {
            ...
            if (fillColor !== null || stroke) {
                ...
                const ptData = int16Array(...verts.flatMap(p => [Math.round(p.x), Math.round(p.y)]));
                this.records.writeRecord(EMR.POLYGON16, concat(int32Array(emfBounds.left, emfBounds.top, emfBounds.right, emfBounds.bottom), uint32Array(verts.length), ptData));
                this.deleteObject(brushHandle);
                this.deleteObject(penHandle);
            }
            this.drawInsetBoxShadows(points, style);
    ...
    drawInsetBoxShadows(points, style) {
        if (!style.boxShadow || !isAxisAlignedRect(points) || style.cornerShapes)
            return;
        ...
        for (const shadow of parseBoxShadow(style.boxShadow)) {
            ...
            const clipPoints = this.buildRoundedRectClipPoints(bounds.x, bounds.y, bounds.w, bounds.h, radius, radius);
            const outerPoints = [
            ...
            const innerPoints = innerW > 0 && innerH > 0
                ? this.buildRoundedRectClipPoints(innerX, innerY, innerW, innerH, Math.min(radius, innerW / 2), Math.min(radius, innerH / 2))
                : [];
            ...
            this.records.writeRecord(EMR.POLYGON16, concat(int32Array(Math.round(bounds.x - pad), Math.round(bounds.y - pad), Math.round(bounds.x + bounds.w + pad), Math.round(bounds.y + bounds.h + pad)), uint32Array(outerPoints.length), int16Array(...outerPoints.flatMap(point => [Math.round(point.x), Math.round(point.y)]))));
            if (innerPoints.length > 0) {
                const innerBounds = this.computeBoundsFromPoints(innerPoints);
                this.records.writeRecord(EMR.POLYGON16, concat(int32Array(innerBounds.left, innerBounds.top, innerBounds.right, innerBounds.bottom), uint32Array(innerPoints.length), int16Array(...innerPoints.flatMap(point => [Math.round(point.x), Math.round(point.y)]))));
            }
    ...
    drawOutline(points, style, outline) {
        ...
        if (isAxisAlignedRect(points) && !style.cornerShapes) {
            ...
            this.records.writeRecord(EMR.ROUNDRECT, int32Array(Math.round(minX - padding), Math.round(minY - padding), Math.round(minX + width + padding), Math.round(minY + height + padding), Math.round(outlineRadius * 2), Math.round(outlineRadius * 2)));
        }
        else {
            const bounds = this.computeBounds(points);
    ...
    async drawPolyline(points, closed, style) {
        ...
        if (style.pathSubpaths?.length) {
            ...
            const penHandle = this.createPenStyled(effectiveStroke, style.strokeDasharray);
            this.selectObject(brushHandle);
            this.selectObject(penHandle);
            if (hasFilledSubpath) {
                ...
                this.records.writeRecord(EMR.SETPOLYFILLMODE, uint32Array(style.fillRule === "evenodd" ? ALTERNATE : WINDING));
        ...
        if (closed && fillColor !== null && points.length >= 3) {
            ...
            const ptData = int16Array(...points.flatMap(p => [Math.round(p.x), Math.round(p.y)]));
            this.records.writeRecord(EMR.POLYGON16, concat(int32Array(bounds.left, bounds.top, bounds.right, bounds.bottom), uint32Array(points.length), ptData));
            this.deleteObject(brushHandle);
            this.deleteObject(penHandle);
        }
        else {
            // Draw as polyline (stroke only)
            const penHandle = this.createPenStyled(stroke ?? { color: fillColor ?? 0, width: 1 }, style.strokeDasharray);
            const nullBrush = this.createBrush(null);
            this.selectObject(nullBrush);
            this.selectObject(penHandle);
            const bounds = this.computeBoundsFromPoints(points);
            const ptData = int16Array(...points.flatMap(p => [Math.round(p.x), Math.round(p.y)]));
            this.records.writeRecord(EMR.POLYLINE16, concat(int32Array(bounds.left, bounds.top, bounds.right, bounds.bottom), uint32Array(points.length), ptData));
            this.deleteObject(nullBrush);
            this.deleteObject(penHandle);
        }
        this.restoreState();
    ...
    async drawImage(quad, dataUrl, width, height, style, rgbData) {
        ...
        if (Math.abs(angle) > 0.01) {
            ...
            this.setWorldTransform(cosA, sinA, -sinA, cosA, cx * (1 - cosA) + cy * sinA, cy * (1 - cosA) - cx * sinA);
        }
    ...
    createBrush(color) {
        ...
        if (color === null) {
            // Null brush (no fill)
            this.records.writeRecord(EMR.CREATEBRUSHINDIRECT, concat(uint32Array(handle), // ihBrush (handle index)
            ...
            this.records.writeRecord(EMR.CREATEBRUSHINDIRECT, concat(uint32Array(handle), uint32Array(BS_SOLID), uint32Array(color), uint32Array(0)));
        }
        return handle;
    }
    ...
    createPen(stroke) {
        const handle = this.allocHandle();
        if (!stroke) {
            this.records.writeRecord(EMR.CREATEPEN, concat(uint32Array(handle), // ihPen
            ...
            this.records.writeRecord(EMR.CREATEPEN, concat(uint32Array(handle), uint32Array(PS_SOLID), int32Array(Math.round(stroke.width), 0), uint32Array(stroke.color)));
        }
        return handle;
    }
    /** Create a pen with an optional dash style from strokeDasharray or border style. */
    createPenStyled(stroke, dasharray, borderStyle) {
        if (!stroke)
            return this.createPen(null);
        ...
        const handle = this.allocHandle();
        this.records.writeRecord(EMR.CREATEPEN, concat(uint32Array(handle), uint32Array(penStyle), int32Array(Math.round(stroke.width), 0), uint32Array(stroke.color)));
        return handle;
    }
    ...
    applyClip(style, bounds) {
        ...
        if (clip) {
            ...
            this.records.writeRecord(EMR.INTERSECTCLIPRECT, int32Array(Math.round(clip.x), Math.round(clip.y), Math.round(clip.x + clip.w), Math.round(clip.y + clip.h)));
            hasClip = true;
        }
        ...
        if (clipShape.kind === "polygon") {
            this.records.writeRecord(EMR.SETPOLYFILLMODE, uint32Array(clipShape.fillRule === "evenodd" ? ALTERNATE : WINDING));
        ...
        switch (clipShape.kind) {
            case "inset":
                this.applyPolygonClipPath(this.buildRoundedRectClipPoints(clipShape.x, clipShape.y, clipShape.w, clipShape.h, clipShape.rx, clipShape.ry), hasClip ? RGN_AND : RGN_COPY);
                break;
            case "ellipse":
                this.applyPolygonClipPath(this.buildEllipseClipPoints(clipShape.cx, clipShape.cy, clipShape.rx, clipShape.ry), hasClip ? RGN_AND : RGN_COPY);
                break;
        }
    }
    applyPolygonClipPath(points, combineMode) {
        const shapeBounds = this.computeBoundsFromPoints(points);
        const ptData = int16Array(...points.flatMap((point) => [Math.round(point.x), Math.round(point.y)]));
        this.records.writeRecord(EMR.BEGINPATH, null);
        this.records.writeRecord(EMR.POLYGON16, concat(int32Array(shapeBounds.left, shapeBounds.top, shapeBounds.right, shapeBounds.bottom), uint32Array(points.length), ptData));
        this.records.writeRecord(EMR.ENDPATH, null);
        this.records.writeRecord(EMR.SELECTCLIPPATH, uint32Array(combineMode));
    }
    applyCompoundClipPath(subpaths, combineMode) {
        const drawableSubpaths = subpaths.filter((subpath) => subpath.points.length >= 2);
        if (drawableSubpaths.length === 0)
            return;
        this.records.writeRecord(EMR.BEGINPATH, null);
        for (const subpath of drawableSubpaths) {
            const shapeBounds = this.computeBoundsFromPoints(subpath.points);
            const ptData = int16Array(...subpath.points.flatMap((point) => [Math.round(point.x), Math.round(point.y)]));
            this.records.writeRecord(EMR.POLYGON16, concat(int32Array(shapeBounds.left, shapeBounds.top, shapeBounds.right, shapeBounds.bottom), uint32Array(subpath.points.length), ptData));
        }
        this.records.writeRecord(EMR.ENDPATH, null);
        this.records.writeRecord(EMR.SELECTCLIPPATH, uint32Array(combineMode));
    }
    buildEllipseClipPoints(cx, cy, rx, ry, segments = 32) {
        const points = [];
    ...
    buildRoundedRectClipPoints(x, y, w, h, rx, ry, segmentsPerCorner = 6) {
    ...
    appendArcPoints(points, cx, cy, rx, ry, startAngle, endAngle, segments) {
    ...
    drawPerSideBorders(points, style) {
        ...
        for (const side of sides) {
            ...
            if (color === null || w <= 0 || !side.borderStyle || side.borderStyle === "none" || side.borderStyle === "hidden")
                continue;
            ...
            if (side.borderStyle === "double" && w >= 3) {
                ...
                this.records.writeRecord(EMR.MOVETOEX, int32Array(Math.round(side.from.x - nx * off), Math.round(side.from.y - ny * off)));
                this.records.writeRecord(EMR.LINETO, int32Array(Math.round(side.to.x - nx * off), Math.round(side.to.y - ny * off)));
                ...
                this.selectObject(nullBrush);
                this.selectObject(penHandle);
                this.records.writeRecord(EMR.MOVETOEX, int32Array(Math.round(side.from.x), Math.round(side.from.y)));
                this.records.writeRecord(EMR.LINETO, int32Array(Math.round(side.to.x), Math.round(side.to.y)));
                this.deleteObject(nullBrush);
                this.deleteObject(penHandle);
            }
        }
    ...
    createFont(height, escapement, faceName, bold, italic) {
        const handle = this.allocHandle();
        ...
        this.records.writeRecord(EMR.EXTCREATEFONTINDIRECTW, concat(uint32Array(handle), extLogFont));
        return handle;
    }
    selectObject(handle) {
    ...
    writeMultiPolylineRecord(type, subpaths) {
        ...
        const pointData = int16Array(...allPoints.flatMap((point) => [Math.round(point.x), Math.round(point.y)]));
        this.records.writeRecord(type, concat(int32Array(bounds.left, bounds.top, bounds.right, bounds.bottom), uint32Array(subpaths.length), uint32Array(allPoints.length), countData, pointData));
    }
    computeBounds(quad) {
...
//# sourceMappingURL=emf-writer.js.map