...
import { getPointBounds, getQuadBounds } from "./shared/clip-path.js";
...
import { formatWriterNumber as n, getVisibleStroke, isAxisAlignedRect } from "./shared/writer-utils.js";
// ── Color helpers ───────────────────────────────────────────────────
/** Convert cornerShapes K-value tuple to a CSS corner-shape value string. */
function cornerShapesToCss(shapes) {
    return shapes.map(k => {
        if (k === Infinity || k >= 10)
            return "square";
        if (k === -Infinity || k <= -10)
...
function injectOpeningTagAttributes(html, attributes) {
    if (attributes.length === 0)
        return html;
    return html.replace(/^<([^\s>/]+)([^>]*?)(\s*\/?)>/, (_match, tagName, rest, closing) => {
...
function subpathsToSvgPath(subpaths) {
    return subpaths
...
function roundedQuadToSvgPath(points, radius) {
...
function clipQuadToCssClipPath(clipQuad) {
...
function normalizeGradientStops(stops) {
    ...
    if (resolved[0].offset < 0)
        resolved[0].offset = 0;
    if (resolved[resolved.length - 1].offset < 0)
...
function buildInlineStrokeGradientDef(strokeImage, id, x, y, w, h) {
    ...
    if (gradient.type === "linear") {
        ...
        const stopsStr = stops.map((stop) => `<stop offset="${n(Math.max(0, Math.min(1, stop.offset)) * 100)}%" stop-color="${escHtml(stop.color)}"/>`).join("");
...
function pushCss(css, name, value) {
    if (!value)
        return;
...
function getVisibleBorder(width, style, color) {
...
function getVisibleBorders(style) {
    const borders = {
...
function appendBorderCss(css, borders, stroke) {
    if (borders) {
        if (borders.top)
            css.push(`border-top:${escHtml(borders.top.width)} ${escHtml(borders.top.style)} ${escHtml(borders.top.color)}`);
        if (borders.right)
            css.push(`border-right:${escHtml(borders.right.width)} ${escHtml(borders.right.style)} ${escHtml(borders.right.color)}`);
        if (borders.bottom)
            css.push(`border-bottom:${escHtml(borders.bottom.width)} ${escHtml(borders.bottom.style)} ${escHtml(borders.bottom.color)}`);
        if (borders.left)
            css.push(`border-left:${escHtml(borders.left.width)} ${escHtml(borders.left.style)} ${escHtml(borders.left.color)}`);
        return;
    }
    if (stroke) {
...
function appendEffectCss(css, style, includeOutline = true) {
    if (includeOutline) {
...
function appendTextCss(css, style, preserveWhitespace, includeLineHeight = true) {
...
function appendTextEffectsCss(css, style) {
    pushCss(css, "filter", style.filter);
    pushCss(css, "mix-blend-mode", style.mixBlendMode);
    if (style.mask) {
        pushCss(css, "mask", style.mask);
...
function getClipBoundsCss(clip) {
    const css = [
        "position:absolute",
        `left:${n(clip.x)}px`,
        `top:${n(clip.y)}px`,
    ...
    if (clip.radius > 0)
        css.push(`border-radius:${n(clip.radius)}px`);
    return css;
}
function offsetCssPosition(style, property, delta) {
    if (Math.abs(delta) < 0.0001)
        return style;
    const pattern = new RegExp(`(^|;)${property}:(-?\\d*\\.?\\d+)(px)?(?=;|$)`);
    if (pattern.test(style)) {
        return style.replace(pattern, (_match, prefix, value) => `${prefix}${property}:${n(parseFloat(value) + delta)}px`);
    }
    const separator = style.length > 0 && !style.endsWith(";") ? ";" : "";
    return `${style}${separator}${property}:${n(delta)}px`;
}
function offsetRootElementPosition(html, dx, dy) {
    if (Math.abs(dx) < 0.0001 && Math.abs(dy) < 0.0001)
        return html;
    return html.replace(/^<([a-zA-Z0-9:-]+)([^>]*?)\sstyle="([^"]*)"([^>]*)>/, (_match, tagName, before, style, after) => {
...
export class HTMLWriter {
    ...
    imageMode;
    imageCounter = 0;
    imageDedup = new Map(); // dataUrl → filename
    cssImageClasses = new Map(); // dataUrl → CSS class name
    customCss;
    /**
     ...
     */
    constructor(optionsOrWidth, height, imageMode, zoom) {
    ...
    pushElement(html, style, bounds, source) {
    ...
    buildSimpleClipGroup(html, style) {
        const clip = style.clipBounds;
        if (!clip)
            return null;
    ...
    async drawPolygon(points, style, source) {
        ...
        if (!fill && !stroke && !borders && !outline && !style.boxShadow && !hasGradient)
            return;
        ...
        if (isAxisAlignedRect(points)) {
            ...
            if (fill)
                css.push(`background-color:${fill}`);
            if (hasGradient)
                css.push(`background-image:${bgImage}`);
            appendBorderCss(css, borders, stroke);
            if (style.borderRadius && style.borderRadius !== "0px")
                css.push(`border-radius:${style.borderRadius}`);
            if (style.cornerShapes)
                css.push(`corner-shape:${cornerShapesToCss(style.cornerShapes)}`);
            if (style.boxShadow && style.boxShadow !== "none")
                css.push(`box-shadow:${style.boxShadow}`);
            if (opacity !== undefined && opacity < 1)
                css.push(`opacity:${n(opacity)}`);
    ...
    async drawPolyline(points, closed, style, source) {
        ...
        const svgAttrs = [];
        if (fill && canFillPath)
            svgAttrs.push(`fill="${escHtml(fill)}"`);
        else
            svgAttrs.push(`fill="none"`);
        if (style.fillRule === "evenodd")
            svgAttrs.push('fill-rule="evenodd"');
        if (stroke) {
        ...
        if (style.strokeDasharray && style.strokeDasharray !== "none")
            svgAttrs.push(`stroke-dasharray="${escHtml(style.strokeDasharray)}"`);
        if (opacity !== undefined && opacity < 1)
            svgAttrs.push(`opacity="${n(opacity)}"`);
    ...
    async end() {
        const content = this.elements.map((entry) => entry.type === "raw"
            ? entry.html
            : `${entry.open}${entry.children.join("")}${entry.close}`).join("\n");
