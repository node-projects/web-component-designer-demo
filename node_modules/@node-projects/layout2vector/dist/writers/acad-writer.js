...
let ACadVersion;
let LwPolyline;
let LwPolylineVertex;
let Hatch;
let HatchBoundaryPath;
let HatchBoundaryPathPolyline;
let HatchPattern;
let HatchPatternTypeEnum;
let TextEntity;
let Color;
let XY;
let XYZ;
...
function cloneStableCadDate() {
    return new Date(STABLE_CAD_DATE_MS);
}
function normalizeCadMetadata(doc) {
    if (doc.header) {
...
function cssToAcadColor(color) {
    const parsed = parseCssColor(color);
    if (!parsed || parsed.a <= 0)
        return undefined;
    return new Color(parsed.r, parsed.g, parsed.b);
}
function toNearestAcadIndexColor(color) {
...
class AcadDocumentBuilder {
    ...
    createPolyline(verts, closed, color) {
    ...
    async drawPolygon(points, style) {
        ...
        if (radius > 0 && isAxisAlignedRect(points) && !style.cornerShapes) {
            ...
            if (fillVisible && fillColor)
                this.addSolidHatchPaths([verts], fillColor);
    ...
    async drawPolyline(points, closed, style) {
        ...
        for (const subpath of subpaths) {
            this.createPolyline(subpath.points.map(point => ({ x: point.x, y: point.y })), subpath.closed, trueColor);
        }
    }
    ...
    async drawImage(_quad, _dataUrl, _width, _height, _style) {
...
async function writeAcadDxfBytes(doc) {
...
//# sourceMappingURL=acad-writer.js.map