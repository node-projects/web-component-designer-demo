...
function getCompatibleFrameMetrics(width, height) {
...
function pointsEqual(a, b, epsilon = 0.01) {
...
async function decodePng(data) {
    ...
    if (!inflated || inflated.byteLength < height * (stride + 1))
        return null;
...
function rgbaToBgraPixels(width, height, rgba) {
...
function rgbToBgrPixels(width, height, rgbData) {
...
function rgbToBgraPixels(width, height, rgbData, alpha) {
    const stride = width * 4;
    const data = new Uint8Array(stride * height);
    for (let index = 0; index < width * height; index += 1) {
...
function scaleRgbaAlpha(rgba, opacity) {
    if (opacity >= 1)
        return rgba;
...
function cssColorToArgb(value, opacity = 1) {
...
function cssColorToArgbStop(value, opacity = 1) {
    const parsed = parseCssColor(value);
    if (!parsed)
        return null;
    const alpha = clamp(Math.round(parsed.a * opacity * 255), 0, 255);
...
function parseVisibleStroke(style, opacity) {
...
function parseVisibleOutline(style, opacity) {
...
function parseFontStyleFlags(style) {
    let flags = 0;
    const weight = (style.fontWeight ?? "").toLowerCase();
    const numericWeight = parseInt(weight, 10);
    if (weight === "bold" || numericWeight >= 600)
    ...
    if (decoration.includes("underline"))
        flags |= FONT_STYLE_UNDERLINE;
    if (decoration.includes("line-through"))
        flags |= FONT_STYLE_STRIKEOUT;
    return flags;
}
function parseTextAlignment(style) {
    const align = (style.textAlign ?? "").toLowerCase();
    if (align === "center")
        return STRING_ALIGNMENT_CENTER;
    if (align === "right" || align === "end")
        return STRING_ALIGNMENT_FAR;
    return STRING_ALIGNMENT_NEAR;
}
function parseStringFormatFlags(style) {
...
function interpolateArgbColor(t, stops, repeating = false) {
...
function ensureGradientEdgeStops(stops, repeating = false) {
...
function resolveGradientStops(stopsAst, scaleLength, repeating) {
    if (scaleLength <= 0)
        return [];
...
function createStrokeFromBorder(color, width, borderStyle, opacity) {
...
function pointArrayToFloat32(points) {
    return float32Array(...points.flatMap((point) => [point.x, point.y]));
}
function quadToTransform(quad) {
...
function transformFigure(figure, matrix) {
    return {
...
function figureFromPoints(points, closed) {
    if (points.length < 2)
        return null;
    return {
        start: points[0],
...
function figureFromRoundedQuad(points, radius, cornerShapes) {
...
function roundedRectFigure(x, y, width, height, rx, ry) {
...
function ellipseFigure(cx, cy, rx, ry) {
    const kappa = 0.5522847498307936;
    const ox = rx * kappa;
    const oy = ry * kappa;
    return {
...
function figuresFromClipShape(shape) {
...
function serializePathFigures(figures, graphicsVersion = GRAPHICS_VERSION) {
    const points = [];
    const pointTypes = [];
    ...
    const pathData = concatBytes(uint32Array(graphicsVersion, points.length, 0), pointArrayToFloat32(points), align4(Uint8Array.from(pointTypes)));
    return pathData;
}
class EmfRecordBuffer {
    chunks = [];
    writeRecord(type, data = EMPTY_BYTES) {
    ...
    writeEmfPlusRecord(type, flags, data = EMPTY_BYTES) {
        ...
        this.writeRecord(EMR.COMMENT, concatBytes(uint32Array(emfPlusRecord.byteLength + 4, EMFPLUS_COMMENT_IDENTIFIER), emfPlusRecord));
    }
    getNumRecords() {
...
export class EMFPlusWriter {
    ...
    async begin() {
        ...
        this.nextStateId = 1;
        this.emitRecord(EMFPLUS.HEADER, EMFPLUS_HEADER_FLAG_DUAL, concatBytes(uint32Array(GRAPHICS_VERSION, EMFPLUS_FLAGS_DISPLAY, DEFAULT_DPI, DEFAULT_DPI)));
    ...
    async drawPolygon(points, style) {
        ...
        if (fillColor === null && gradientFills.length === 0 && !stroke)
            return;
        this.saveState();
        this.applyClip(style, clipBounds);
        this.emitBackgroundFills([figure], fillColor, gradientFills, backgroundBox);
    ...
    async drawImage(quad, dataUrl, width, height, style, rgbData) {
        ...
        this.emitDrawImagePoints(IMAGE_SLOT, DEFAULT_IMAGE_ATTRIBUTES_SLOT, [quad[0], quad[1], quad[3]], imageSource.width, imageSource.height);
        this.restoreState();
    }
    async end() {
    ...
    emitRecord(type, flags = 0, data = EMPTY_BYTES) {
        this.records.writeEmfPlusRecord(type, flags, data);
    }
    emitObject(slot, objectType, data) {
        this.emitRecord(EMFPLUS.OBJECT, slot | (objectType << 8), data);
    }
    emitSetPageTransform(pageScale, pageUnit) {
        this.emitRecord(EMFPLUS.SET_PAGE_TRANSFORM, pageUnit, float32Array(pageScale));
    }
    emitSetWorldTransform(matrix) {
        this.emitRecord(EMFPLUS.SET_WORLD_TRANSFORM, 0, float32Array(...matrix));
    }
    emitDrawString(fontSlot, stringFormatSlot, color, text, width, height) {
        const stringBytes = align4(encodeUtf16LE(text));
        const data = concatBytes(uint32Array(color, stringFormatSlot, text.length), float32Array(0, 0, width, height), stringBytes);
        this.emitRecord(EMFPLUS.DRAW_STRING, fontSlot | 0x8000, data);
    }
    emitDrawImagePoints(imageSlot, imageAttributesSlot, points, sourceWidth, sourceHeight) {
        const flags = imageSlot;
        const data = concatBytes(uint32Array(imageAttributesSlot, UNIT_PIXEL), float32Array(0, 0, sourceWidth, sourceHeight), uint32Array(3), pointArrayToFloat32(points));
    ...
    applyClip(style, bounds) {
        let hasClip = false;
        if (style.clipQuads?.length) {
            for (const clipQuad of style.clipQuads) {
                this.applyClipFigures([clipQuad.radius > 0 ? figureFromRoundedQuad(clipQuad.points, clipQuad.radius) : this.figureForClipQuad(clipQuad.points)], hasClip ? COMBINE_MODE_INTERSECT : COMBINE_MODE_REPLACE);
                hasClip = true;
            }
        }
        if (style.clipBounds) {
            this.emitRecord(EMFPLUS.SET_CLIP_RECT, (hasClip ? COMBINE_MODE_INTERSECT : COMBINE_MODE_REPLACE) << 8, float32Array(style.clipBounds.x, style.clipBounds.y, style.clipBounds.w, style.clipBounds.h));
            hasClip = true;
        }
    ...
    applyClipFigures(figures, combineMode) {
        if (figures.length === 0)
            return;
    ...
    buildBackgroundGradientFills(backgroundImage, opacity, box) {
        return parseAllGradientsAst(backgroundImage, {
            parseColor: (value) => cssColorToArgbStop(value, opacity),
        });
    }
    buildPresetColorData(stops) {
        return concatBytes(uint32Array(stops.length), float32Array(...stops.map((stop) => clamp(stop.offset, 0, 1))), uint32Array(...stops.map((stop) => stop.color)));
    }
    buildLinearGradientBrush(gradient, box) {
        ...
        return concatBytes(uint32Array(BRUSH_GRAPHICS_VERSION, BRUSH_TYPE_LINEAR_GRADIENT, BRUSH_DATA_TRANSFORM | (hasPresetStops ? BRUSH_DATA_PRESET_COLORS : 0), 0), float32Array(0, -0.5, 1, 1), uint32Array(firstStop.color, lastStop.color, firstStop.color, lastStop.color), float32Array(worldEnd.x - worldStart.x, worldEnd.y - worldStart.y, worldStart.y - worldEnd.y, worldEnd.x - worldStart.x, worldStart.x, worldStart.y), hasPresetStops ? this.buildPresetColorData(stops) : EMPTY_BYTES);
    }
    buildRadialGradientBrush(gradient, box) {
        ...
        const lastStop = stops[stops.length - 1];
        const hasPresetStops = stops.length > 2;
        return concatBytes(uint32Array(BRUSH_GRAPHICS_VERSION, BRUSH_TYPE_PATH_GRADIENT, BRUSH_DATA_PATH | (hasPresetStops ? BRUSH_DATA_PRESET_COLORS : 0), WRAP_MODE_CLAMP, firstStop.color), float32Array(worldCenter.x, worldCenter.y), uint32Array(1, lastStop.color, pathData.byteLength), pathData, hasPresetStops ? this.buildPresetColorData(stops) : EMPTY_BYTES);
    }
    emitBrushFilledFigures(figures, brushData) {
        if (figures.length === 0)
            return;
        this.emitObject(PATH_SLOT, OBJECT_TYPE.PATH, serializePathFigures(figures));
    ...
    emitConicGradientFill(figures, gradient, box) {
    ...
    emitBackgroundFills(figures, fillColor, gradients, box) {
        if (fillColor !== null) {
            this.emitFigures(figures, fillColor, null, undefined, undefined);
        }
        if (!box || gradients.length === 0)
            return;
        for (let index = gradients.length - 1; index >= 0; index -= 1) {
            ...
            if (gradient.type === "linear") {
                ...
                if (brushData)
                    this.emitBrushFilledFigures(figures, brushData);
                continue;
            }
    ...
    emitFigures(figures, fillColor, stroke, _fillRule, dasharray) {
        if (figures.length === 0)
            return;
        this.emitObject(PATH_SLOT, OBJECT_TYPE.PATH, serializePathFigures(figures));
        if (fillColor !== null) {
    ...
    buildPenObject(stroke, dasharray, borderStyle) {
        let penFlags = 0;
        ...
        return concatBytes(uint32Array(GRAPHICS_VERSION, 0, penFlags, UNIT_PIXEL), float32Array(stroke.width), optionalData, brushData);
    }
    buildFontObject(fontSize, style) {
        const familyName = align4(encodeUtf16LE(parseFontFamily(style.fontFamily)));
        return concatBytes(uint32Array(GRAPHICS_VERSION), float32Array(fontSize), uint32Array(UNIT_PIXEL, parseFontStyleFlags(style), 0, familyName.byteLength / 2), familyName);
    }
    buildStringFormatObject(style) {
        return concatBytes(uint32Array(GRAPHICS_VERSION, parseStringFormatFlags(style), 0, parseTextAlignment(style), STRING_ALIGNMENT_NEAR, STRING_DIGIT_SUBSTITUTION_USER, 0), float32Array(0, 0, 0, 0, 1), uint32Array(STRING_TRIMMING_NONE), int32Array(0, 0));
    }
    buildImageAttributesObject() {
        return concatBytes(uint32Array(GRAPHICS_VERSION, 0, WRAP_MODE_CLAMP, 0, OBJECT_CLAMP_RECT, 0));
    }
    buildImageObject(image) {
        const bitmapData = image.compressed
            ? concatBytes(uint32Array(image.width, image.height), int32Array(0), uint32Array(PIXEL_FORMAT_UNDEFINED, BITMAP_DATA_TYPE_COMPRESSED), image.data)
            : concatBytes(uint32Array(image.width, image.height), int32Array(image.stride), uint32Array(image.pixelFormat, BITMAP_DATA_TYPE_PIXEL), image.data);
        return concatBytes(uint32Array(GRAPHICS_VERSION, IMAGE_DATA_TYPE_BITMAP), bitmapData);
    }
    async resolveImageSource(dataUrl, width, height, opacity, rgbData) {
    ...
    looksLikePng(data) {
        if (!data || data.byteLength < PNG_SIGNATURE.byteLength)
            return false;
        for (let index = 0; index < PNG_SIGNATURE.byteLength; index += 1) {
            if (data[index] !== PNG_SIGNATURE[index])
                return false;
    ...
    hasMixedBorders(style) {
        ...
        if (!sides[0].borderStyle)
            return false;
        if (sides.some((side) => side.borderStyle === "double"))
            return true;
    ...
    drawPerSideBorders(points, style, opacity) {
        const sides = [
...
//# sourceMappingURL=emfplus-writer.js.map