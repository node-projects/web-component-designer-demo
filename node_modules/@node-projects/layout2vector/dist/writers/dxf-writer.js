...
let point3d;
let point2d;
let HatchBoundaryPaths;
let HatchPolylineBoundary;
let HatchPredefinedPatterns;
let pattern;
...
function normalizeImageBasePath(basePath) {
...
export class DXFWriter {
    ...
    getTextStyle(fontFamily) {
        ...
        if (!name || name === "serif" || name === "sans-serif" || name === "monospace")
            return undefined;
        if (this.fontStyles.has(name))
            return this.fontStyles.get(name);
    ...
    addSolidHatchPaths(paths, fillColor) {
        ...
        if (!hasBoundary)
            return;
        this.dxf.addHatch(boundary, Pattern({ name: PredefinedPatterns.SOLID }), { trueColor: String(fillColor) });
    }
        ...
        if (radius > 0 && (!isAxisAlignedRect(points) || style.cornerShapes)) {
            ...
            if (fillVisible) {
                ...
                if (fillColor2 !== undefined)
                    this.addSolidHatchPaths([verts], fillColor2);
            }
    ...
    async drawText(quad, text, style) {
        ...
        if (trueColor !== undefined)
            opts.trueColor = String(trueColor);
        if (Math.abs(angleDeg) > 0.1)
            opts.rotation = angleDeg;
        const Point3d = point3d;
        const textEntity = this.dxf.addText(Point3d(bottomLeft.x, this.flipY(bottomLeft.y)), height, sanitized, Object.keys(opts).length > 0 ? opts : undefined);
    ...
    async drawImage(quad, dataUrl, width, height, _style) {
        ...
        const Point3d = point3d;
        this.dxf.addImage(imagePath, `image${idx}`, Point3d(x, y, 0), width, height, scale, angleDeg);
    }
    async end() {
