...
export class SVGWriter {
    ...
    async drawPolygon(points, style, source) {
        ...
        if (isAxisAlignedRect(points) && !style.cornerShapes) {
            ...
            const baseElement = this.buildLayeredShape((attrs) => `<rect x="${n(x)}" y="${n(y)}" width="${n(w)}" height="${n(h)}" rx="${n(r)}" ry="${n(r)}"${attrs}/>`, fill, mixedBorders ? null : stroke, style, gradientIds, undefined, undefined);
        ...
        const baseElement = this.buildLayeredShape((attrs) => `<path d="${pathD}"${attrs}/>`, fill, mixedBorders ? null : stroke, style, gradientIds, undefined, undefined);
        const layers = [baseElement];
        if (outline) {
            layers.push(this.buildOutlineElement(points, style, outline, undefined));
        }
    ...
    async drawPolyline(points, closed, style, source) {
        ...
        const element = this.buildLayeredShape((attrs) => `<path d="${d}"${style.fillRule === "evenodd" ? ' fill-rule="evenodd"' : ""}${attrs}/>`, hasClosedSubpath ? fill : null, stroke, style, gradientIds, undefined, opacity);
    ...
    buildLayeredShape(createShape, fill, stroke, style, gradientIds, filterId, opacity) {
        const layers = [];
        if (fill) {
