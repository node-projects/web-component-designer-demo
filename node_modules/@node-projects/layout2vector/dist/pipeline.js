...
function inheritContainerClipping(nodes, inheritedStyle) {
...
function getPseudoInheritedStyle(node) {
...
function attachSourceMetadata(nodes, element, options, originalType) {
...
export async function extractIR(root, options = {}) {
    ...
    for (const rootEl of roots) {
        ...
        const stackingTree = traverseDOM(rootEl, options.includeInvisible ?? false, options.walkIframes ?? false, { rootScrollBehavior: options.rootScrollBehavior });
...
export async function extractIRWithAssets(root, options = {}) {
...
function forEachPolylinePoint(node, visitor) {
    const seen = new Set();
    for (const point of node.points) {
        if (seen.has(point))
            continue;
...
function isVisibleNode(node) {
    switch (node.type) {
        ...
        case "polyline": {
            const s = node.style;
            if ((node.closed || !!s.pathSubpaths?.length) && isVisibleColor(s.fill))
                return true;
            if (isVisibleColor(s.stroke) && s.strokeWidth && parseFloat(s.strokeWidth) > 0)
                return true;
            return false;
...
function getPointCollectionBounds(points) {
    let left = Infinity;
    let top = Infinity;
    let right = -Infinity;
    let bottom = -Infinity;
...
function isFullyClippedNode(node) {
