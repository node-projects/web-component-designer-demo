...
const sampledPathGeometryCache = new Map();
/**
 * Extract geometry from an entire SVG subtree.
 * SVG DOM order defines paint order (z-index does not apply inside SVG).
 */
export function extractSVGSubtree(svgRoot, baseZIndex, options, htmlParentOpacity = 1) {
    const results = [];
...
function walkSVGTree(el, results, nextIndex, options, parentOpacity) {
...
function extractSVGElement(el, zIndex, options, effectiveOpacity, preComputedStyle) {
    ...
    switch (tag) {
        case "rect":
            return extractRect(el, style, zIndex, ctm);
        case "circle":
            return extractCircle(el, style, zIndex, ctm);
        case "ellipse":
            return extractEllipse(el, style, zIndex, ctm);
        case "line":
            return extractLine(el, style, zIndex, ctm);
        case "polyline":
            return extractPolyline(el, style, zIndex, false, ctm);
        case "polygon":
            return extractPolyline(el, style, zIndex, true, ctm);
        case "path":
            return extractPath(el, style, zIndex, ctm);
        case "text":
            if (options.includeText !== false) {
                return extractText(el, style, zIndex, ctm);
            }
            return [];
        default:
            return [];
...
function extractSVGStyle(cs, el, ctm) {
...
function resolveGradientColor(urlRef, el) {
...
function resolveGradient(urlRef, el) {
    const match = urlRef.match(/url\(["']?#([^"')]+)["']?\)/);
    if (!match)
        return undefined;
    const id = match[1];
...
function transformPoints(points, el, preCtm) {
...
function extractRect(el, style, zIndex, ctm) {
    ...
    const transformed = transformPoints(rawQuad, el, ctm);
    return [{ type: "polygon", points: transformed, style, zIndex }];
}
function extractCircle(el, style, zIndex, ctm) {
...
function extractEllipse(el, style, zIndex, ctm) {
    const cx = el.cx.baseVal.value;
    const cy = el.cy.baseVal.value;
...
function extractLine(el, style, zIndex, ctm) {
    const p1 = { x: el.x1.baseVal.value, y: el.y1.baseVal.value };
    const p2 = { x: el.x2.baseVal.value, y: el.y2.baseVal.value };
    const transformed = transformPoints([p1, p2], el, ctm);
    const results = [{ type: "polyline", points: transformed, closed: false, style, zIndex }];
    results.push(...extractMarkers(el, transformed, style, zIndex, false));
    return results;
}
function extractPolyline(el, style, zIndex, closed, ctm) {
    const points = [];
    ...
    if (points.length === 0)
        return [];
    const transformed = transformPoints(points, el, ctm);
    const results = [{ type: "polyline", points: transformed, closed, style, zIndex }];
    results.push(...extractMarkers(el, transformed, style, zIndex, closed));
    return results;
}
function extractPath(el, style, zIndex, ctm) {
...
function splitPathSubpaths(el) {
    if (typeof el.getPathData === "function") {
        try {
            const rawSegments = el.getPathData({ normalize: true });
...
function splitNormalizedPathData(segments) {
    const subpaths = [];
    let current = [];
...
function extractCompoundPathBySampling(el, subpaths, style, zIndex, ctm) {
    const ownerSvg = el.ownerSVGElement;
    if (!ownerSvg)
        return [];
    const pathSubpaths = [];
...
function extractPathFromPathData(el, style, zIndex, ctm) {
    // getPathData returns normalized path segments
    // Still sample for consistent output
    return extractPathBySampling(el, style, zIndex, ctm);
}
function extractPathBySampling(el, style, zIndex, ctm) {
...
function samplePathGeometry(pathEl, transformEl, ctm) {
    ...
    if (sampled === undefined) {
        ...
        try {
            ...
            sampled = null;
            if (pathData)
                sampledPathGeometryCache.set(pathData, sampled);
            return null;
        }
        if (totalLength === 0) {
            sampled = null;
 ...
 */
function extractMarkers(el, points, style, zIndex, closed = false) {
    if (points.length < 2)
        return [];
    const cs = getComputedStyle(el);
    ...
    function resolveMarker(ref) {
        ...
        if (!m)
            return null;
        return ownerSvg.querySelector(`#${m[1]}`);
    }
    function placeMarker(marker, pos, angle) {
        ...
        for (const child of Array.from(marker.children)) {
            if (child instanceof SVGPathElement) {
                ...
                const childCs = getComputedStyle(child);
                const childFill = childCs.fill || child.getAttribute("fill") || undefined;
                const markerStyle = { ...style, fill: childFill !== "none" ? childFill : undefined };
                ...
                const cx0 = child instanceof SVGCircleElement ? child.cx.baseVal.value : child.cx.baseVal.value;
                const cy0 = child instanceof SVGCircleElement ? child.cy.baseVal.value : child.cy.baseVal.value;
                const rx0 = child instanceof SVGCircleElement ? child.r.baseVal.value : child.rx.baseVal.value;
                const ry0 = child instanceof SVGCircleElement ? child.r.baseVal.value : child.ry.baseVal.value;
                const circPts = [];
                ...
                const childFill = childCs.fill || child.getAttribute("fill") || undefined;
                const markerStyle = { ...style, fill: childFill !== "none" ? childFill : undefined };
                results.push({ type: "polygon", points: transformed, style: markerStyle, zIndex });
            }
        }
    }
...
function extractText(el, style, zIndex, ctm) {
    const bbox = el.getBBox();
    if (bbox.width === 0 && bbox.height === 0)
        return [];
...
//# sourceMappingURL=svg-extractor.js.map