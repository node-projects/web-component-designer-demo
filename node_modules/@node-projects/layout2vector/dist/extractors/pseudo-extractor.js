 ...
 * to extract.
 */
export function extractPseudoElements(el, parentStyle, globalIndex, options) {
    const results = [];
...
const DEFAULT_LIST_MARKER_TEXT = {
    disc: "\u2022",
    circle: "\u25e6",
    square: "\u25aa",
    "disclosure-open": "\u25be",
 ...
 */
export function parseCSSContentValue(content, el) {
    if (!content || content === "none" || content === "normal")
        return null;
    if (/\burl\s*\(/.test(content))
        return null;
    ...
    if (!el)
        return tokens.filter(t => t.type === "string").map(t => t.value).join("") || null;
    let result = "";
 ...
 */
function parseCounterProp(value, defaultVal) {
    ...
    while (i < parts.length) {
        const name = parts[i];
        if (name === "none" || name === "initial" || name === "inherit") {
            i++;
            continue;
        }
        i++;
 ...
 */
function resolveCounter(el, name) {
 ...
 */
function walkCounterScope(scopeEl, targetEl, name, startValue) {
 ...
 */
function resolveCountersValue(el, name, separator, listStyle) {
    ...
    if (scopes.length === 0)
        return formatCounter(0, listStyle);
    const values = [];
 ...
 */
function formatCounter(value, listStyle) {
    switch (listStyle) {
        case "lower-alpha":
        case "lower-latin":
...
function resolveDefaultMarkerText(el, listStyleType) {
...
function resolveMarkerText(el, markerCs) {
...
function extractMarkerPseudo(el, parentStyle, globalIndex, options) {
    const elStyle = getComputedStyle(el);
    if (elStyle.display !== "list-item")
        return [];
    const markerCs = getComputedStyle(el, "::marker");
    if (markerCs.display === "none")
        return [];
    if (markerCs.visibility === "hidden")
        return [];
    if (markerCs.opacity === "0")
        return [];
    ...
    if (parentStyle.clipBounds)
        markerStyle.clipBounds = parentStyle.clipBounds;
    if (parentStyle.clipQuads)
        markerStyle.clipQuads = parentStyle.clipQuads;
    return [{
            type: "text",
            quad,
            text,
...
// ─── quote resolution ────────────────────────────────────────
function resolveQuote(el, isOpen) {
...
function extractOnePseudo(el, pseudo, parentStyle, globalIndex, options) {
    const pseudoCs = getComputedStyle(el, pseudo);
    // Skip when pseudo-element isn't generated
    if (pseudoCs.display === "none")
        return [];
    if (pseudoCs.visibility === "hidden")
        return [];
    if (pseudoCs.opacity === "0")
        return [];
    ...
    if (text === null)
        return []; // non-text or unresolvable
    ...
    if (parentStyle.clipBounds)
        pseudoStyle.clipBounds = parentStyle.clipBounds;
    if (parentStyle.clipQuads)
        pseudoStyle.clipQuads = parentStyle.clipQuads;
    ...
    const quads = getElementQuads(temp, boxType);
    const results = [];
...
//# sourceMappingURL=pseudo-extractor.js.map