 ...
 */
export function extractBackgroundImage(el, style, globalIndex, _options) {
    const bg = style.backgroundImage;
    if (!bg || bg === "none")
        return [];
...
export function extractMaskedElementImage(el, style, globalIndex, options) {
    const quad = getElementQuad(el);
    if (!quad)
        return [];
    const htmlEl = el;
...
function renderMaskedElement(el, style, w, h) {
    try {
        const cs = getComputedStyle(el);
 ...
 */
export function extractImageGeometry(el, style, globalIndex, options) {
    ...
    try {
        ...
        if (xhr.status === 200 || xhr.status === 0) {
            ...
            if (raw.charCodeAt(0) === 0xFF && raw.charCodeAt(1) === 0xD8)
                mime = "image/jpeg";
            else if (raw.charCodeAt(0) === 0x47 && raw.charCodeAt(1) === 0x49)
                mime = "image/gif";
