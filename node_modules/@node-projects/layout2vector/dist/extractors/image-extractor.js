...
export function isCanvasElement(el) {
...
function getConfiguredTimeout(name, fallbackMs) {
    const configured = globalThis[name];
...
function runImageLoadTask(key, load) {
...
async function createDecodedImage(src) {
    try {
        const img = new Image();
        img.src = src;
...
async function fetchExternalImageDataUrl(url) {
    const cached = preloadedUrlMap.get(url);
    if (cached?.startsWith("data:image/"))
        return cached;
    try {
        const resp = await fetchWithTimeout(url, getConfiguredTimeout("__HC_IMAGE_FETCH_TIMEOUT_MS", IMAGE_FETCH_TIMEOUT_MS), {
            redirect: "follow",
        });
        if (!resp.ok)
            return null;
...
async function ensureImageElementSourceReady(src) {
    if (!src)
        return;
    if (src.startsWith("data:")) {
        if (!src.startsWith("data:image/") || preloadedImageElems.has(src))
            return;
        await runImageLoadTask(`img-data:${src}`, async () => {
            if (preloadedImageElems.has(src))
                return;
    ...
    if (preloadedImageElems.has(src) && preloadedUrlMap.has(src))
        return;
...
function getPreloadableStyles(el) {
...
function collectCssImageUrls(cs) {
    const urls = new Set();
    for (const value of [
        cs.backgroundImage,
...
async function preloadCssImageUrl(url) {
    if (!url)
        return;
    if (url.startsWith("data:")) {
        if (!url.startsWith("data:image/"))
            return;
        if (preloadedImageElems.has(url))
            return;
        await runImageLoadTask(`css-data:${url}`, async () => {
            ...
            if (decoded)
                preloadedImageElems.set(url, decoded);
        });
        return;
    }
    ...
    if (cachedUrl) {
        if (cachedUrl.startsWith("data:image/") && !preloadedImageElems.has(cachedUrl)) {
            await runImageLoadTask(`css-decoded:${cachedUrl}`, async () => {
                if (preloadedImageElems.has(cachedUrl))
                    return;
                const decoded = await createDecodedImage(cachedUrl);
                if (decoded)
                    preloadedImageElems.set(cachedUrl, decoded);
            });
        }
        return;
    }
    await runImageLoadTask(`css:${url}`, async () => {
        if (preloadedUrlMap.has(url))
            return;
...
function extractCssUrlValue(value) {
 ...
 */
export async function extractBackgroundImage(el, style, globalIndex, _options) {
    const bg = style.backgroundImage;
    if (!bg || bg === "none")
        return [];
...
export async function extractMaskedElementImage(el, style, globalIndex, options) {
    const quad = getElementQuad(el);
    if (!quad)
        return [];
    const htmlEl = el;
...
function getRasterizedMaskStyle(style) {
    return {
        ...style,
        mask: undefined,
 ...
 */
function rasterToRendered(dataUrl, w, h) {
...
function inspectRasterImageData(imageData, includeRgbData) {
...
function rasterToRenderedUncached(dataUrl, w, h) {
    try {
...
function renderDataUrlWithSampling(dataUrl, w, h, disableSmoothing, sourceRect) {
    try {
        const img = new Image();
        img.src = dataUrl;
        if (!img.complete || img.naturalWidth === 0)
...
function decodeBgSvgDataUrl(dataUrl) {
    try {
...
function decodeSvgDataPayload(payload) {
...
function parseSvgViewBoxSize(value) {
    if (!value)
        return null;
...
function resolveSvgRasterSourceSize(src, boxWidth, boxHeight) {
 ...
 */
function remapIRNodes(nodes, w, h, targetQuad) {
 ...
 */
function usesEvenOddFillRule(svgEl) {
    ...
    for (const el of Array.from(svgEl.querySelectorAll("*"))) {
        ...
        if (/fill-rule\s*:\s*evenodd/i.test(style))
            return true;
    }
...
function convertBgSvgToGeometry(svgContent, el, targetQuad, globalIndex, options) {
    const parser = new DOMParser();
    const doc = parser.parseFromString(stripXmlPreamble(svgContent), "image/svg+xml");
    const parsedSvg = doc.documentElement;
    ...
    if (!options.svgToVector && usesEvenOddFillRule(parsedSvg))
        return [];
    const tempSvg = document.importNode(parsedSvg, true);
...
function renderBackgroundImage(el, url, elWidth, elHeight) {
...
function parseBackgroundRepeat(value) {
    const tokens = value.split(/\s+/).filter(Boolean);
    if (tokens.length === 0)
        return { repeatX: true, repeatY: true };
    const parseAxis = (token) => token !== "no-repeat";
    if (tokens.length === 1) {
        switch (tokens[0]) {
            case "repeat-x":
...
function parseBackgroundLengthToken(token, reference, scale) {
    if (!token || token === "auto")
        return null;
    if (token.endsWith("%")) {
...
function resolveBackgroundSize(value, imageWidth, imageHeight, boxWidth, boxHeight, scaleX, scaleY) {
    const tokens = value.split(/\s+/).filter(Boolean);
...
function parseBackgroundPositionToken(token, axis) {
    if (!token)
        return { type: "percent", value: axis === "x" ? 0 : 0 };
    switch (token) {
        case "left":
        case "top":
...
function resolveBackgroundPosition(value) {
    const tokens = value.split(/\s+/).filter(Boolean);
    if (tokens.length === 0) {
        return {
...
function resolveBackgroundOffset(spec, boxSize, tileSize, scale) {
    if (spec.type === "percent")
        return (boxSize - tileSize) * spec.value;
    return spec.value * scale;
}
function getRepeatStart(offset, step) {
    if (!Number.isFinite(step) || step <= 0)
        return offset;
    const remainder = offset % step;
    return remainder <= 0 ? remainder : remainder - step;
}
function renderBackgroundImageUncached(el, url, w, h, repeatValue, sizeValue, positionValue, attachmentValue) {
    try {
        let source = null;
        let sourceWidth = 0;
        let sourceHeight = 0;
        ...
        if (!source) {
            ...
            if (!sourceWidth || !sourceHeight || !img.complete)
                return null;
        }
...
async function renderMaskedElement(el, style, w, h) {
    try {
        const cs = getComputedStyle(el);
...
function resolveMaskFillColor(style, cs) {
...
function requiresRasterSvgImage(style) {
 ...
 */
export async function extractImageGeometry(el, style, globalIndex, options) {
    ...
    if (!isDataUrl && !preloadedEl && (!el.complete || el.naturalWidth === 0))
        return [];
    const quad = getElementQuad(el);
    if (!quad)
        return [];
    ...
    if (cachedImg !== undefined) {
        if (cachedImg === null)
            return []; // previously determined to be transparent/empty
        return [{
                type: "image",
                quad: adjustedQuad,
    ...
    if (isSvgSource(src) && src.startsWith("data:image/svg+xml"))
        dataUrl = src;
 ...
 */
export function extractCanvasGeometry(el, style, globalIndex, options) {
 ...
 */
export async function extractVideoGeometry(el, style, globalIndex, options) {
 ...
 */
function getObjectFitPlan(el, quad, natW, natH) {
...
function getObjectFitPlanFromDimensions(quad, natW, natH, objectFit, objectPositionValue) {
    if (!natW || !natH)
        return { quad };
...
async function loadFirstVideoFrame(el, src) {
    const canUseOriginalElement = canReuseOriginalVideoElement(el, src);
    if (canUseOriginalElement &&
    ...
    if (crossOriginMode)
        tempVideo.crossOrigin = crossOriginMode;
    tempVideo.src = src;
    try {
...
function canReuseOriginalVideoElement(el, src) {
    if (el.crossOrigin)
        return true;
    return !isCrossOriginVideoSource(src);
}
function getVideoCrossOriginMode(el, src) {
    if (el.crossOrigin === "use-credentials")
        return "use-credentials";
    if (el.crossOrigin === "anonymous")
        return "anonymous";
    return isCrossOriginVideoSource(src) ? "anonymous" : null;
}
function isCrossOriginVideoSource(src) {
    try {
...
async function seekVideoToStart(video) {
    video.pause();
...
function disposeTemporaryVideo(video) {
    try {
        video.pause();
    }
    catch {
...
function parseObjectPosition(value) {
    ...
    for (let index = 0; index < tokens.length; index += 1) {
        ...
        if (!isObjectPositionLength(token))
            continue;
...
function isObjectPositionLength(token) {
    return /^[-+]?\d*\.?\d+(?:[a-z%]+)?$/i.test(token) || isCssCalcExpression(token);
}
function parseObjectPositionLength(token) {
...
function createKeywordAxisPosition(keyword, offsetToken) {
...
function resolveAxisPosition(position, freeSpace) {
...
function isCssCalcExpression(token) {
    const value = token.trim().toLowerCase();
    return value.startsWith("calc(") && value.endsWith(")");
}
function parseCssLengthExpression(token) {
    const value = token.trim().toLowerCase();
    if (!value)
        return null;
...
function parseObjectViewBox(value, natW, natH) {
...
function normalizeSourceRect(rect, natW, natH) {
...
function composeSourceRects(baseRect, cropRect) {
    if (!baseRect)
        return cropRect;
    if (!cropRect)
        return baseRect;
...
function fitQuadWithinBox(quad, boxW, boxH, natW, natH, objectPosition) {
    const scale = Math.min(boxW / natW, boxH / natH);
    return placeQuadInsideBox(quad, boxW, boxH, natW * scale, natH * scale, objectPosition);
}
function placeQuadInsideBox(quad, boxW, boxH, renderW, renderH, objectPosition) {
...
function subQuad(quad, startU, startV, endU, endV) {
    const lerp = (a, b, t) => ({
        x: a.x + (b.x - a.x) * t,
        y: a.y + (b.y - a.y) * t,
    });
...
function drawObjectFitImage(ctx, source, renderW, renderH, sourceRect) {
    if (sourceRect) {
...
function extractSvgContent(src) {
 ...
 */
function convertSvgToGeometry(svgContent, imgEl, targetQuad, globalIndex, options) {
...
function decodePngDataUrl(dataUrl) {
    try {
        ...
        const ihdrLen = readU32(bytes, offset);
        offset += 4;
        ...
        offset += 4;
        const height = readU32(bytes, offset);
        offset += 4;
        ...
        offset += 4; // CRC
        if (bitDepth !== 8 || interlace !== 0 || compression !== 0 || filter !== 0)
            return null;
        ...
        if (channels < 0)
            return null; // indexed not supported
        // Collect all IDAT chunks
        const idatChunks = [];
        while (offset < bytes.length) {
            const chunkLen = readU32(bytes, offset);
            offset += 4;
        ...
        if (!rawData || rawData.length < rowBytes)
            return null; // need at least 1 row
        const rgba = new Uint8ClampedArray(width * height * 4);
        const prev = new Uint8Array(width * bpp); // previous scanline
        for (let y = 0; y < height; y++) {
            const rowStart = y * rowBytes;
            if (rowStart + 1 + width * bpp > rawData.length)
                break; // truncated data — remaining rows stay transparent
            const filterType = rawData[rowStart];
            if (filterType > 4)
                break; // invalid filter byte — data is corrupt from here
