...
function createsOverflowClip(overflow, ignoreScrollableOverflow = false) {
...
export function extractStyle(cs) {
    ...
    let fill = bgColor && bgColor !== "rgba(0, 0, 0, 0)" && bgColor !== "transparent"
        ? bgColor
        : undefined;
...
function parseCornerShapeValue(v) {
    if (!v)
        return 1;
    const trimmed = v.trim();
    if (!trimmed || trimmed === "round")
        return 1;
...
function pickComputedMaskValue(cs) {
    const candidates = [
...
function isWithinClosedDetailsContent(element) {
 ...
 */
export function traverseDOM(root, includeInvisible = false, walkIframes = false, options = {}) {
    return buildStackingNode(root, includeInvisible, 1, undefined, undefined, IDENTITY_TRANSFORM, walkIframes, undefined, root, options.rootScrollBehavior ?? "clip");
}
function buildStackingNode(element, includeInvisible, parentOpacity, parentClipBounds, parentClipPath, coordinateTransform, walkIframes, parentClipQuads, traversalRoot, rootScrollBehavior) {
    ...
    const shouldExpandRootScrollContent = element === traversalRoot
    ...
    if (createsOverflowClip(overflowX, shouldExpandRootScrollContent)
    ...
    if (walkIframes && isHTMLIFrameElement(element)) {
        ...
        if (iframeRoot && iframeViewport) {
            ...
            if (includeInvisible || isVisible(iframeRootStyle)) {
                ...
                node.children.push(buildStackingNode(iframeRoot, includeInvisible, effectiveOpacity, intersectClipBounds(childClipBounds, iframeViewport.clipBounds), childClipPath, iframeViewport.transform, walkIframes, iframeClipQuads, traversalRoot, rootScrollBehavior));
            }
        }
    }
    return node;
}
function buildTextStackingNode(element, style, extractedStyle, textNode, zIndex, coordinateTransform, clipQuads, clipBounds, childClipBounds) {
    return {
        element,
        style,
...
function preserveSpecifiedClipPath(element, style) {
    const specifiedClipPath = getSpecifiedClipPathValue(element);
    if (!specifiedClipPath)
        return;
    if (!style.clipPath) {
...
function getSpecifiedClipPathValue(element) {
    ...
    if (rawInlineClipPath)
        return rawInlineClipPath;
    if (element instanceof HTMLElement || element instanceof SVGElement) {
        return (element.style.getPropertyValue("clip-path") || element.style.getPropertyValue("-webkit-clip-path")).trim();
    }
    return "";
}
function extractInlineStyleProperty(styleText, propertyName) {
    const match = styleText.match(new RegExp(`(?:^|;)\\s*${propertyName}\\s*:\\s*([^;]+)`, "i"));
    return match ? match[1].trim() : "";
}
function getScrollExpandedChildTransform(element, coordinateTransform) {
    const scrollLeft = typeof element.scrollLeft === "number"
        ? element.scrollLeft ?? 0
        : 0;
    const scrollTop = typeof element.scrollTop === "number"
        ? element.scrollTop ?? 0
        : 0;
...
function getElementScrollTranslation(element, scrollLeft, scrollTop) {
...
function getIframeTraversalRoot(element) {
    try {
...
function getIframeViewport(element, cs, coordinateTransform) {
    ...
    if (contentQuad) {
        ...
        if (viewportWidth <= 0 || viewportHeight <= 0)
            return null;
        const viewportQuad = rectToQuad(rect.left + element.clientLeft, rect.top + element.clientTop, viewportWidth, viewportHeight);
...
function transformQuad(quad, coordinateTransform) {
    return quad.map((point) => applyCoordinateTransform(point, coordinateTransform));
}
...
function intersectClipBounds(parent, child) {
    if (!parent)
        return child;
    if (!child)
        return parent;
    ...
    const epsilon = 0.01;
    const matchesParent = Math.abs(parent.x - x) < epsilon &&
        Math.abs(parent.y - y) < epsilon &&
        Math.abs(parent.w - w) < epsilon &&
        Math.abs(parent.h - h) < epsilon;
    const matchesChild = Math.abs(child.x - x) < epsilon &&
...
function composeCoordinateTransforms(outer, inner) {
    return {
...
function getTransformedRectBounds(rect, coordinateTransform) {
...
function paintsInAutoOrZeroLayer(node) {
    if (node.textOnly)
        return false;
...
function collectInOrder(node, result) {
    ...
    for (const child of node.children) {
        ...
        if (paintsInAutoOrZeroLayer(child))
            hasAutoOrZeroLayer = true;
    }
