...
function createsOverflowClip(overflow) {
 ...
 */
export function traverseDOM(root, includeInvisible = false, walkIframes = false) {
    return buildStackingNode(root, includeInvisible, 1, undefined, undefined, IDENTITY_TRANSFORM, walkIframes, undefined);
}
function buildStackingNode(element, includeInvisible, parentOpacity, parentClipBounds, parentClipPath, coordinateTransform, walkIframes, parentClipQuads) {
    ...
    if (walkIframes && isHTMLIFrameElement(element)) {
        ...
        if (iframeRoot && iframeViewport) {
            ...
            if (includeInvisible || isVisible(iframeRootStyle)) {
                ...
                node.children.push(buildStackingNode(iframeRoot, includeInvisible, effectiveOpacity, intersectClipBounds(childClipBounds, iframeViewport.clipBounds), childClipPath, iframeViewport.transform, walkIframes, iframeClipQuads));
            }
        }
    }
    return node;
