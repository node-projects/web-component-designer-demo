...
function parseBorderRadius(borderRadius, w, h) {
...
function extractFirstGradient(bgImage) {
    ...
    for (let i = match.index; i < bgImage.length; i++) {
        if (bgImage[i] === "(")
            ...
            depth--;
            if (depth === 0)
                return bgImage.slice(match.index, i + 1);
        }
    }
    return null;
}
...
export class SVGWriter {
    ...
    defs = [];
    defIdCounter = 0;
    clipCache = new Map(); // clipKey → clip-path attribute string
