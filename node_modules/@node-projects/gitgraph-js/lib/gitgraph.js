import { GitgraphCore, MergeStyle, arrowSvgPath, toSvgPath, Mode, Orientation, TemplateName, templateExtend, } from "@node-projects/gitgraph-core";
import { createSvg, createG, createText, createCircle, createUse, createPath, createClipPath, createDefs, createForeignObject, } from "./svg-elements.js";
import { createBranchLabel, PADDING_X as BRANCH_LABEL_PADDING_X, PADDING_Y as BRANCH_LABEL_PADDING_Y, } from "./branch-label.js";
import { createTag, PADDING_X as TAG_PADDING_X } from "./tag.js";
import { createTooltip, PADDING as TOOLTIP_PADDING } from "./tooltip.js";
...
function createGitgraph(graphContainer, options) {
    ...
    function adaptSvgOnUpdate(adaptToContainer) {
        ...
        function computeOffsets() {
            const commits = Array.from($commits.children);
            let totalOffsetY = 0;
            // In VerticalReverse orientation, commits are in the same order in the DOM.
            const orientedCommits = gitgraph.orientation === Orientation.VerticalReverse
                ? commits
                : commits.reverse();
            commitYWithOffsets = orientedCommits.reduce((newOffsets, commit) => {
    ...
    function renderBranchesPaths(branchesPaths) {
        ...
        const paths = Array.from(branchesPaths).map(([branch, coordinates]) => {
            return createPath({
                d: toSvgPath(coordinates.map((coordinate) => coordinate.map(getWithCommitOffset)), isBezier, gitgraph.isVertical),
                fill: "none",
    ...
    function getWithCommitOffset({ x, y }) {
