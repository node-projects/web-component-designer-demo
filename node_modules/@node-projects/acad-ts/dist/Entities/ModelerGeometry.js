...
export class ModelerGeometry extends Entity {
    ...
    /** @internal */
    guid = '';
    /** @internal */ acisTextEncoded = null;
    modelerFormatVersion = 0;
    /** Raw SAT/SAB modeler payload when it is stored inline in the entity. */
    binaryData = new Uint8Array(0);
    get acisData() { return this.binaryData; }
    set acisData(value) { this.binaryData = value; }
    get isBinaryAcisData() {
        const signature = 'ACIS BinaryFile';
        if (this.binaryData.length < signature.length)
            return false;
        for (let i = 0; i < signature.length; i++) {
            if (this.binaryData[i] !== signature.charCodeAt(i))
                return false;
        }
        return true;
    }
    getAcisText() {
        if (this.binaryData.length === 0 || this.isBinaryAcisData)
            return null;
