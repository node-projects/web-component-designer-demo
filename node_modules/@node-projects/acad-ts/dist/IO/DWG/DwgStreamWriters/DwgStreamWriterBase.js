...
export class DwgStreamWriterBase {
    ...
    static getMergedWriter(version, stream, encoding) {
        ...
        if (mergedKey === 'MergedAC14') {
            const LegacyFactory = MergedFactory;
            return new LegacyFactory(stream, new WriterFactory(stream, encoding), new WriterFactory(new Uint8Array(0), encoding));
        }
        else {
            const SplitFactory = MergedFactory;
            return new SplitFactory(stream, new WriterFactory(stream, encoding), new WriterFactory(new Uint8Array(0), encoding), new WriterFactory(new Uint8Array(0), encoding));
        }
    }
    writeInt(value) {
