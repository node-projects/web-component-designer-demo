...
export class DxfDocumentBuilder extends CadDocumentBuilder {
    get ignoreProxyGraphics() { return true; }
    acdsDataRecords = new Map();
    configuration;
    ...
    buildDocument() {
        ...
        for (const [handle, payload] of this.acdsDataRecords) {
            const object = this.tryGetCadObject(handle);
            if (object instanceof ModelerGeometry)
                object.acisData = payload;
            else
                this.notify(`ACDSDATA record owner ${handle} is not a ModelerGeometry entity in the document`, NotificationType.Warning);
        }
