...
export class DocumentContainer extends BaseCustomWebComponentLazyAppend {
  ...
  executeCommand(command) {
    if (this._tabControl.selectedIndex === 0) this.designerView.executeCommand(command);else if (this._tabControl.selectedIndex === 1) this.codeView.executeCommand(command);
  }

  canExecuteCommand(command) {
    if (this._tabControl.selectedIndex === 0) return this.designerView.canExecuteCommand(command);else if (this._tabControl.selectedIndex === 1) return this.codeView.canExecuteCommand(command);
    return false;
  }
  ...
  set content(value) {
    ...
    if (this._tabControl) {
      if (this._tabControl.selectedIndex === 0) this.designerView.parseHTML(this._content);else if (this._tabControl.selectedIndex === 1) this.codeView.update(this._content);else if (this._tabControl.selectedIndex === 2) this.demoView.display(this._serviceContainer, this.designerView.instanceServiceContainer, this._content);
    }
  }

  get content() {
    if (this._tabControl) {
      if (this._tabControl.selectedIndex === 0) this._content = this.designerView.getHTML();else if (this._tabControl.selectedIndex === 1) this._content = this.codeView.getText();
  ...
  ready() {
    this._tabControl.onSelectedTabChanged.on(i => {
      ...
      } else if (i.oldIndex === 1) this._content = this.codeView.getText();

      if (i.newIndex === 0) this.designerView.parseHTML(this._content);else if (i.newIndex === 1) {
      ...
      } else if (i.newIndex === 2) this.demoView.display(this._serviceContainer, this.designerView.instanceServiceContainer, this._content);
    });

    if (this._content) this.content = this._content;
  }
