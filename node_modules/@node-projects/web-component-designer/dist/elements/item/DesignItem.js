...
export class DesignItem {
  constructor(node, serviceContainer, instanceServiceContainer) {
    ...
    if (node instanceof HTMLElement || node instanceof SVGElement) {
      for (let s of node.style) {
        ...
        if (val && typeof val === 'string') this.styles.set(s, node.style[s]);
      }

      if (!this._lockAtDesignTime) node.style.pointerEvents = 'auto';else node.style.pointerEvents = 'none'; //node.style.cursor = 'pointer';
    }
  ...
  *children() {
  ...
  static GetOrCreateDesignItem(node, serviceContainer, instanceServiceContainer) {
    if (!node) return null;
    let designItem = node[DesignItem._designItemSymbol];
