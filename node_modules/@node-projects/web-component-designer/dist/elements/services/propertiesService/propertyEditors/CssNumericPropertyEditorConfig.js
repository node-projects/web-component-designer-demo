...
const inferredCssNumericPropertyNames = {
    length: new Set([
        'width',
        'height',
        'minWidth',
...
export function getCssNumericPropertyType(type, propertyName) {
    if (type === 'css-length')
        return 'length';
    ...
    if (!camelName)
        return null;
    for (const numericType of Object.keys(inferredCssNumericPropertyNames)) {
        if (inferredCssNumericPropertyNames[numericType].has(camelName))
            return numericType;
    }
    return null;
}
export function isCssNumericPropertyType(type, propertyName) {
    return getCssNumericPropertyType(type, propertyName) != null;
}
export function convertCssNumericUnitValue(context) {
    const decimalPlaces = getNumericValueDecimalPlaces(context);
...
//# sourceMappingURL=CssNumericPropertyEditorConfig.js.map