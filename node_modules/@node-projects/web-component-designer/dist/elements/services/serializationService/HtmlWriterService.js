  ...
  write(indentedTextWriter, designItem, options, designItemsAssignmentList) {
    ...
    if (designItem.nodeType == NodeType.TextNode) {
      ...
      if (designItem.hasAttributes) {
        for (const a of designItem.attributes) {
          ...
          if (a[1]) indentedTextWriter.write(a[0] + '="' + DomConverter.normalizeAttributeValue(a[1]) + '"');
        }
      }
      ...
      if (designItem.hasStyles) {
        ...
        for (const s of styles) {
          if (s[0]) indentedTextWriter.write(s[0] + ':' + DomConverter.normalizeAttributeValue(s[1]) + ';');
        }
      ...
      if (designItem.hasChildren) {
        ...
        for (const c of designItem.children()) {
          c.serviceContainer.forSomeServicesTillResult('htmlWriterService', s => {
            if (s.canWrite(c)) s.write(indentedTextWriter, c, options, designItemsAssignmentList);
          });
        }
    ...
    if (designItemsAssignmentList) {
      designItemsAssignmentList.set(designItem, {
        start: start,
        length: indentedTextWriter.position - start
      });
    }
