...
export class TreeView extends BaseCustomWebComponentLazyAppend {
  ...
  /*, activeElement: Element */
    ...
    if (rootItem != null) this._recomputeTree(rootItem.element, null
    /*, activeElement */
    );
  ...
  _selectTreeElements(items) {
    if (this._previouslySelected) {
      for (let e of this._previouslySelected) if (e) e.classList.remove('selected');
    }

    this._previouslySelected = items;
    if (items) for (let e of items) if (e) e.classList.add('selected');
  }
  ...
  _highlight(element) {
    if (!element) return; // Find it in the tree.
