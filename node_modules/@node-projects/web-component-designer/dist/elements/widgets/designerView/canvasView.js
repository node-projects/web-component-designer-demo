...
/** @class */
(() => {
  class CanvasView extends BaseCustomWebComponent {
    constructor() {
      super(); // Settings
    ...
    async _onDrop(event) {
      ...
      this.instanceServiceContainer.undoService.execute(new InsertAction(this.rootDesignItem, this._canvas.children.length, di)); //this._canvas.appendChild(instance);
    }
    ...
    _selectedElementsChanged(selectionChangedEvent) {
      if (selectionChangedEvent.oldSelectedElements) {
        for (let e of selectionChangedEvent.oldSelectedElements) e.element.classList.remove(CanvasView._activeClassName);
      }

      if (selectionChangedEvent.selectedElements) {
        for (let e of selectionChangedEvent.selectedElements) e.element.classList.add(CanvasView._activeClassName);
      }
    }
  ...
  CanvasView._activeClassName = 'active';
  CanvasView.style = css`
    :host {
      display: block;
