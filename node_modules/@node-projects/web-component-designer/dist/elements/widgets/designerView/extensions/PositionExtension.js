...
export class PositionExtension extends AbstractExtension {
    ...
    _locks = [];
    _texts = [];
    constructor(extensionManager, designerView, extendedItem, _options = {}) {
    ...
    refresh() {
        ...
        const container = (absolute ? element.offsetParent : null) ?? this.extendedItem.parent?.element;
        if (!container)
            return;
        ...
        if (this._valuesHaveChanges(scale, absolute, editable, ...anchored, ...distances, ...canvasStarts.flatMap(p => [p.x, p.y]), ...canvasEnds.flatMap(p => [p.x, p.y]), ...centers.flatMap(p => [p.x, p.y]))) {
    ...
    _drawLock(index, x, y, anchored, editable) {
    ...
    _switchDockingSide(side) {
        ...
        const axisSides = horizontal ? ['left', 'right'] : ['top', 'bottom'];
        const container = this.extendedItem.element.offsetParent;
        if (container && axisSides.every(side => this._getSpecifiedStyle(side) !== 'auto')) {
            ...
            if (target === ignoredSide) {
                ...
                const number = (css, name) => parseFloat(css.getPropertyValue(name)) || 0;
                const sum = (css, prefix, suffix = '') => axisSides.reduce((value, side) => value + number(css, prefix + side + suffix), 0);
