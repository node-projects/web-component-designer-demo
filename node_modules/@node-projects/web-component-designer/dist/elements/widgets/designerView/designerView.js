...
export class DesignerView extends BaseCustomWebComponentLazyAppend {
  ...
  onKeyDown(event) {
    ...
    */
    if ((event.ctrlKey || event.metaKey) && event.key === 'z' && !event.shiftKey) this.handleCommand('undo');else if ((event.ctrlKey || event.metaKey) && event.key === 'z' && event.shiftKey) this.handleCommand('redo');else {
  ...
  _pointerEventHandlerElement(event, currentElement, forcedAction, actionMode) {
    ...
    if (event.type != EventNames.PointerUp && currentElement.parentNode == this.svgLayer) return;
    if (currentElement.parentNode == this.svgLayer) currentElement = (_a = this.instanceServiceContainer.selectionService.primarySelection.element) !== null && _a !== void 0 ? _a : this._canvas;
    ...
    if (designItems && designItems.length) {
      for (let i of designItems) {
        if (i.nodeType == NodeType.Element) {
          ...
          line.setAttribute('x', p.x - this._containerBoundingRect.x);
          line.setAttribute('width', p.width);
          line.setAttribute('y', p.y - this._containerBoundingRect.y);
          line.setAttribute('height', p.height);
  ...
  _drawMoveOverlay(posX, posY, w, h, designItem) {
    ...
    line.addEventListener(EventNames.PointerDown, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Drag));
    line.addEventListener(EventNames.PointerMove, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Drag));
    line.addEventListener(EventNames.PointerUp, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Drag));
    this.svgLayer.appendChild(line);
  ...
  _drawResizerOverlay(posX, posY, cursor, designItem) {
    ...
    line.setAttribute('cx', posX);
    line.setAttribute('cy', posY);
    line.setAttribute('r', 3);
    line.setAttribute('class', 'svg-selection svg-primary-resizer');
    line.setAttribute('style', 'cursor: ' + cursor);
    line.addEventListener(EventNames.PointerDown, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Resize, cursor));
    line.addEventListener(EventNames.PointerMove, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Resize, cursor));
    line.addEventListener(EventNames.PointerUp, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Resize, cursor));
    this.svgLayer.appendChild(line);
  }
