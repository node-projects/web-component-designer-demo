...
import { BaseCustomWebComponentLazyAppend, css, html, DomHelper } from "../../../../node_modules/@node-projects/base-custom-webcomponent/dist/index.js";
...
export class DesignerView extends BaseCustomWebComponentLazyAppend {
  ...
  _createDesignItemsRecursive(element) {
  ...
  _pointerEventHandlerElement(event, currentElement) {
    switch (event.type) {
    ...
    if (this._actionType == null) {
      ...
      if (event.type == EventNames.PointerDown) {
        ...
        if (currentDesignItem !== this.rootDesignItem && this._forceMove(currentPoint, {
          x: rectCurrentElement.left - this._ownBoundingRect.left,
          y: rectCurrentElement.top - this._ownBoundingRect.top
        })) {
          ...
          }) ? PointerActionType.Resize : PointerActionType.DragOrSelect;
        }
      }
    }
  ...
  _pointerActionTypeDragOrSelect(event, currentDesignItem, currentPoint) {
    ...
    switch (event.type) {
      ...
      case EventNames.PointerMove:
        {
          if (currentPoint.x != this._initialPoint.x || currentPoint.y != this._initialPoint.y) this._actionType = PointerActionType.Drag;
          if (this._actionType != PointerActionType.Drag) return;
          ...
          this._drawOutlineRects(this.instanceServiceContainer.selectionService.selectedElements); //todo -> what is if a transform already exists -> backup existing style.?
  ...
  _forceMove(pointerPoint, elementPoint) {
    return pointerPoint.x < elementPoint.x && pointerPoint.y < elementPoint.y;
  }

  _drawOutlineRects(selectedElements, mode = 'none') {
    ...
    if (selectedElements && selectedElements.length) {
      ...
      line.setAttribute('x', p0.x - this._containerBoundingRect.x - 12);
      line.setAttribute('width', 10);
      line.setAttribute('y', p0.y - this._containerBoundingRect.y - 12);
      line.setAttribute('height', 10);
      line.setAttribute('class', 'svg-selection svg-primary-selection-move');
      line.addEventListener(EventNames.PointerDown, event => this._pointerEventHandlerElement(event, selectedElements[0].element));
      line.addEventListener(EventNames.PointerMove, event => this._pointerEventHandlerElement(event, selectedElements[0].element));
      line.addEventListener(EventNames.PointerUp, event => this._pointerEventHandlerElement(event, selectedElements[0].element));
