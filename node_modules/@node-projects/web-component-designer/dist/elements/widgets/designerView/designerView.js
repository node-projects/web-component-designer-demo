...
import { BaseCustomWebComponentLazyAppend, css, html, DomHelper } from "../../../../../base-custom-webcomponent/dist/index.js";
...
export class DesignerView extends BaseCustomWebComponentLazyAppend {
  constructor() {
    ...
    this.svgLayer = this._getDomElement('svg');
  ...
  _pointerEventHandler(event) {
    ...
    if (currentElement === this._outercanvas2 || currentElement === this.svgLayer) currentElement = this._canvas;
  ...
  _pointerEventHandlerElement(event, currentElement, forcedAction, actionMode) {
    var _a;

    if (event.type != EventNames.PointerUp && currentElement && currentElement.parentNode == this.svgLayer) return;
    if (currentElement && currentElement.parentNode == this.svgLayer) currentElement = (_a = this.instanceServiceContainer.selectionService.primarySelection.element) !== null && _a !== void 0 ? _a : this._canvas;
  ...
  _pointerActionTypeDrawSelection(event, currentElement, currentPoint) {
    ...
    if (event.type == EventNames.PointerUp) {
      ...
      for (let e of elements) {
        ...
        if (elementRect.top - this._containerBoundingRect.top >= oy1 && elementRect.left - this._containerBoundingRect.left >= ox1 && elementRect.top - this._containerBoundingRect.top + elementRect.height <= oy2 && elementRect.left - this._containerBoundingRect.left + elementRect.width <= ox2) {
  ...
  redrawOverlays(designItems) {}

  _drawOutlineRects(designItems, mode = 'none') {
    ...
    if (designItems && designItems.length) {
      for (let i of designItems) {
        if (i.nodeType == NodeType.Element) {
          ...
          rect.setAttribute('x', p.x - this._containerBoundingRect.x);
          rect.setAttribute('width', p.width);
          rect.setAttribute('y', p.y - this._containerBoundingRect.y);
  ...
  _drawMoveOverlay(posX, posY, w, h, designItem) {
    ...
    rect.addEventListener(EventNames.PointerDown, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Drag));
    rect.addEventListener(EventNames.PointerMove, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Drag));
    rect.addEventListener(EventNames.PointerUp, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Drag));
    this.svgLayer.appendChild(rect);
    const text = document.createElementNS("http://www.w3.org/2000/svg", "text");
    text.setAttribute('x', posX);
    text.setAttribute('y', posY + 11);
  ...
  _drawResizerOverlay(posX, posY, cursor, designItem) {
    ...
    circle.addEventListener(EventNames.PointerDown, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Resize, cursor));
    circle.addEventListener(EventNames.PointerMove, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Resize, cursor));
    circle.addEventListener(EventNames.PointerUp, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Resize, cursor));
    this.svgLayer.appendChild(circle);
  }

  _drawRotateOverlay(posX, posY, designItem) {
    ...
    g2.appendChild(path);
    g.appendChild(g2);
    g.addEventListener(EventNames.PointerDown, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Rotate));
    g.addEventListener(EventNames.PointerMove, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Rotate));
    g.addEventListener(EventNames.PointerUp, event => this._pointerEventHandlerElement(event, designItem.element, PointerActionType.Rotate));
