...
export class DesignerView extends BaseCustomWebComponentLazyAppend {
  ...
  set additionalStyle(value) {
    ...
    } else this.shadowRoot.adoptedStyleSheets = [this.constructor.style];
  }
  ...
  async executeCommand(command) {
    switch (command.type) {
      case CommandType.screenshot:
        {
          if (!this.instanceServiceContainer.selectionService.primarySelection) alert("you need to select an elment!");else {
  ...
  async parseHTML(html) {
    ...
    const parserService = this.serviceContainer.htmlParserService;
    if (!html) DomHelper.removeAllChildnodes(this.rootDesignItem.element);else {
  ...
  _onWheel(event) {
    if (event.ctrlKey) {
      ...
      this.zoomFactorChanged(); //TODO: we should zoom on the current cursor position, so it stays the center
    }
  }
  ...
  _pointerEventHandler(event) {
    ...
    if (this._lastHoverDesignItem != currentDesignItem) {
      if (this._lastHoverDesignItem) this.extensionManager.removeExtension(this._lastHoverDesignItem, ExtensionType.MouseOver);
      if (currentDesignItem && currentDesignItem != this.rootDesignItem && currentElement.parentNode !== this.overlayLayer) this.extensionManager.applyExtension(currentDesignItem, ExtensionType.MouseOver);
    ...
    let tool = (_b = this.serviceContainer.globalContext.tool) !== null && _b !== void 0 ? _b : this.serviceContainer.designerTools.get(NamedTools.Pointer);
