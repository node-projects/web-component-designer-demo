...
export class DesignerView extends BaseCustomWebComponentLazyAppend {
  ...
  _pointerEventHandlerElement(event, currentElement, forcedAction) {
    if (currentElement.parentNode == this.svgLayer) return;
    ...
    if (this._actionType == null) {
      ...
      if (event.type == EventNames.PointerDown) {
        ...
        /* this._forceMove({ x: currentPoint.containerOriginalX, y: currentPoint.containerOriginalY }, { x: rectCurrentElement.left - this._ownBoundingRect.left, y: rectCurrentElement.top - this._ownBoundingRect.top })*/
          ...
          this._actionType = isCurrentItemSelected && this._shouldResize(currentPoint, {
  ...
  _drawOutlineRects(designItems, mode = 'none') {
    ...
    if (designItems && designItems.length) {
      ...
      line.addEventListener(EventNames.PointerDown, event => this._pointerEventHandlerElement(event, designItems[0].element, PointerActionType.Drag));
      line.addEventListener(EventNames.PointerMove, event => this._pointerEventHandlerElement(event, designItems[0].element, PointerActionType.Drag));
      line.addEventListener(EventNames.PointerUp, event => this._pointerEventHandlerElement(event, designItems[0].element, PointerActionType.Drag));
