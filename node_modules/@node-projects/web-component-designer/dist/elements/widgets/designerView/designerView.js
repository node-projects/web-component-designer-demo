...
export class DesignerView extends BaseCustomWebComponent {
  ...
  constructor() {
    ...
    this._canvasContainer = this._getDomElement('canvasContainer'); //this._outerCanvas2 = this._getDomElement<HTMLDivElement>('outercanvas2');
  ...
  onKeyDown(event) {
    ...
    let oldTop = parseInt(primarySelection.element.style.top); //let oldPosition = (<HTMLElement>primarySelection.element).style.position;
  ...
  _selectedElementsChanged(selectionChangedEvent) {
    if (selectionChangedEvent.oldSelectedElements) {
      for (let e of selectionChangedEvent.oldSelectedElements) this._toggleDesignerClass(e, 'active', false);
    }

    if (selectionChangedEvent.selectedElements) {
      for (let e of selectionChangedEvent.selectedElements) this._toggleDesignerClass(e, 'active', true);
    }
  }

  _toggleDesignerClass(designItem, className, state) {
    if (state) {
      ...
      if (designItem.element.classList.contains(DesignerView._designerClassPrefix + className)) {
        ...
        if (designItem.element.classList.length === 0) designItem.element.removeAttribute('class');
      }
    }
  }
  ...
  getDesignerMousepoint(event, startPoint) {
  ...
  _pointerEventHandler(event) {
    ...
    this._ownBoundingRect = this.getBoundingClientRect(); //this._containerBoundingRect = this._outerCanvas2.getBoundingClientRect();
    ...
    if (this._actionType == null) {
      ...
      if (event.type == EventNames.PointerDown) {
        ...
        if (this._forceMove(currentPoint, {
    ...
    if (event.type == EventNames.PointerUp) {
      ...
      if (this._actionType == PointerActionType.DrawSelection) {
        if (currentElement !== this.rootDesignItem.element) this.setSelectedElements([currentElement]);
      }
  ...
  _pointerActionTypeDrawSelection(event, currentElement, currentPoint) {
    ...
    if (event.type == EventNames.PointerUp) {
      ...
      for (let e of elements) {
        ...
        if (elementRect.top - this._containerBoundingRect.top >= y1 && elementRect.left - this._containerBoundingRect.left >= x1 && elementRect.top - this._containerBoundingRect.top + elementRect.height <= y2 && elementRect.left - this._containerBoundingRect.left + elementRect.width <= x2) {
      ...
      case EventNames.PointerMove:
        {
          ...
          containerService.place(this, currentDesignItem.parent, this._initialPoint, currentPoint, this.instanceServiceContainer.selectionService.selectedElements); //todo -> what is if a transform already exists -> backup existing style.?
...
DesignerView._designerClassPrefix = 'node-projects-wcdesigner-';
