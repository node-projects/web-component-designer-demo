...
(function (factory) {
  ...
  function autoScroll(tree, event) {
    ...
    if (sp !== document && sp.tagName !== "HTML") {
      ...
      if (spOfs.top + sp.offsetHeight - event.pageY < sensitivity) {
        delta = sp.scrollHeight - tree.$scrollParent.innerHeight() - scrollTop; // console.log ("sp.offsetHeight: " + sp.offsetHeight
        // 	+ ", spOfs.top: " + spOfs.top
        // 	+ ", scrollTop: " + scrollTop
  ...
  function evalEffectModifiers(tree, event, effectDefault) {
    ...
    if (res !== SUGGESTED_DROP_EFFECT) {
      tree.info("evalEffectModifiers: " + event.type + " - evalEffectModifiers(): " + SUGGESTED_DROP_EFFECT + " -> " + res);
    }

    SUGGESTED_DROP_EFFECT = res; // tree.debug("evalEffectModifiers: " + res);
  ...
  function prepareDropEffectCallback(event, data) {
    ...
    data.dropEffectSuggested = evalEffectModifiers(tree, event, tree.options.dnd5.dropEffectDefault);
    data.isMove = data.dropEffect === "move";
    data.files = dataTransfer.files || []; // if (REQUESTED_EFFECT_ALLOWED !== dataTransfer.effectAllowed) {
    // 	tree.warn(
  ...
  function applyDropEffectCallback(event, data, allowDrop) {
    ...
    if (event.type !== "dragstart" && REQUESTED_EFFECT_ALLOWED !== data.effectAllowed) {
      tree.warn("effectAllowed should only be changed in dragstart event: " + event.type + ": data.effectAllowed changed from " + REQUESTED_EFFECT_ALLOWED + " -> " + data.effectAllowed);
    }
    ...
    data.isMove = data.dropEffect === "move"; // data.isMove = data.dropEffectSuggested === "move";

    REQUESTED_EFFECT_ALLOWED = data.effectAllowed;
    REQUESTED_DROP_EFFECT = data.dropEffect; // if (REQUESTED_DROP_EFFECT !== dataTransfer.dropEffect) {
    // 	data.tree.info(
    // 		"applyDropEffectCallback(" +
    // 			event.type +
    ...
    dataTransfer.dropEffect = REQUESTED_DROP_EFFECT; // tree.debug(
    // 	"applyDropEffectCallback(" +
    // 		event.type +
    // 		"): set " +
    // 		dataTransfer.dropEffect +
  ...
  function handleDragOver(event, data) {
    ...
    if (hitMode === "after" || hitMode === "before" || hitMode === "over") {
      ...
      if (options.rtl) {
        ...
        pos.at = "right " + markerAt; // console.log("rtl", pos);
      }

      $dropMarker.toggleClass(classDropAfter, hitMode === "after").toggleClass(classDropOver, hitMode === "over").toggleClass(classDropBefore, hitMode === "before").show().position(FT.fixPositionOptions(pos));
    } else {
      $dropMarker.hide(); // console.log("hide dropmarker")
    }

    $(targetNode.span).toggleClass(classDropTarget, hitMode === "after" || hitMode === "before" || hitMode === "over").toggleClass(classDropAfter, hitMode === "after").toggleClass(classDropBefore, hitMode === "before").toggleClass(classDropAccept, hitMode === "over").toggleClass(classDropReject, hitMode === false);
    return hitMode;
  }
  /*
  ...
  function onDragEvent(event) {
    ...
    switch (event.type) {
      case "dragstart":
        ...
        SOURCE_NODE = node; // Also optionally store selected nodes
        ...
        $sourceList.addClass(classDragSource); // Set payload
        // Note:
        ...
        prepareDropEffectCallback(event, data); // Let user cancel or modify above settings
        ...
        applyDropEffectCallback(event, data); // Unless user set `data.useDefaultImage` to false in dragStart,
        ...
        if (data.useDefaultImage) {
          ...
          if (SOURCE_NODE_LIST && SOURCE_NODE_LIST.length > 1) {
            ...
            $extraHelper = $("<span class='fancytree-childcounter'/>").text("+" + (SOURCE_NODE_LIST.length - 1)).appendTo($dragImage);
          }
      ...
      case "dragend":
        ...
        dndOpts.dragEnd(node, data, !LAST_HIT_MODE); // applyDropEffectCallback(event, data);
  ...
  function onDropEvent(event) {
    ...
    switch (event.type) {
      case "dragenter":
        ...
        $(node.span).addClass(classDropOver).removeClass(classDropAccept + " " + classDropReject); // Data is only readable in the dragstart and drop event,
        ...
        if (dndOpts.preventNonNodes && !isSourceFtNode) {
          ...
          DRAG_ENTER_RESPONSE = false;
          break;
        } else if (dndOpts.preventSameParent && data.otherNode && data.otherNode.tree === node.tree && node.parent === data.otherNode.parent) {
          node.debug("Reject dropping as sibling (same parent).");
          DRAG_ENTER_RESPONSE = false;
          break;
        } else if (dndOpts.preventRecursion && data.otherNode && data.otherNode.tree === node.tree && node.isDescendantOf(data.otherNode)) {
        ...
        $dropMarker.show(); // Call dragEnter() to figure out if (and where) dropping is allowed

        prepareDropEffectCallback(event, data);
        r = dndOpts.dragEnter(node, data);
        res = normalizeDragEnterResponse(r); // alert("res:" + JSON.stringify(res))
      ...
      case "dragover":
        ...
        prepareDropEffectCallback(event, data);
        LAST_HIT_MODE = handleDragOver(event, data); // The flag controls the preventDefault() below:
        ...
        if (allowAutoExpand && !node.expanded && node.hasChildren() !== false) {
          ...
          } else if (dndOpts.autoExpandMS && Date.now() - DRAG_OVER_STAMP > dndOpts.autoExpandMS && !node.isLoading() && (!dndOpts.dragExpand || dndOpts.dragExpand(node, data) !== false)) {
            node.setExpanded();
          }
        } else {
          DRAG_OVER_STAMP = null;
      ...
      case "drop":
        ...
        if (nodeData) {
          ...
          } catch (ex) {// assume 'text' type contains plain text, so `otherNodeData`
            // should not be set
          }
        }

        tree.debug(event.type + ": nodeData: '" + nodeData + "', otherNodeData: ", data.otherNodeData);
        $(node.span).removeClass(classDropOver + " " + classDropAccept + " " + classDropReject); // Let user implement the actual drop operation
        ...
        dndOpts.dragDrop(node, data); // applyDropEffectCallback(event, data);
        ...
        if (orgSourceElem && !document.body.contains(orgSourceElem)) {
          ...
          if (orgSourceTree === tree) {
            ...
            tree.warn("Drop handler removed source element: dragend event may be lost.");
          }
        }
  ...
  $.ui.fancytree.registerExtension({
    ...
    options: {
      ...
      // Prevent dropping items on unloaded lazy Fancytree nodes
    ...
    treeInit: function (ctx) {
      ...
      this.$container.addClass("fancytree-ext-dnd5"); // Store the current scroll parent, which may be the tree
      ...
      if (!$dropMarker.length) {
        $dropMarker = $("<div id='fancytree-drop-marker'></div>").hide().css({
          "z-index": 1000,
      ...
      $dropMarker.toggleClass("fancytree-rtl", !!opts.rtl); // Enable drag support if dragStart() is specified:
