...
(function (factory) {
  ...
  $.ui.fancytree._FancytreeClass.prototype._fixStart = function (start, apply) {
    var vp = this.viewport,
        nodeList = this.visibleNodeList;
    start = start == null ? vp.start : start; // this.debug("_fixStart(" + start + ", " + !!apply + ")");

    var orgStart = start; // Don't scroll down below bottom node
  ...
  $.ui.fancytree._FancytreeClass.prototype._shiftViewport = function (mode, ofs) {
  ...
  $.ui.fancytree._FancytreeClass.prototype.isViewportBottom = function () {
    return this.viewport.start + this.viewport.count >= this.visibleNodeList.length;
  };
  /**
  ...
  $.ui.fancytree._FancytreeClass.prototype.setViewport = function (opts) {
    ...
    var i,
      ...
      force: null
        ...
        trCount = trList.length; // Sanitize viewport settings and check if we need to redraw
    ...
    start = opts.start == null ? vp.start : Math.max(0, +opts.start); // Adjust start value to assure the current content is inside vp
    ...
    if (!opts.noEvents && this._triggerTreeEvent("beforeUpdateViewport", null, info) === false) {
      return false;
    }

    info.prev = $.extend({}, vp);
    delete info.next; // vp.enabled = newVp.enabled;

    vp.start = newVp.start;
    vp.count = newVp.count; // Make sure we have the correct count of TRs
    ...
    trCount = trList.length; // Update visible node cache if needed
  ...
  $.ui.fancytree._FancytreeClass.prototype.adjustViewportSize = function () {
    _assert(this.scrollWrapper, "No parent div.fancytree-grid-container found.");
    ...
    var $table = this.$container,
        ...
        newCount = trHeight ? Math.floor(free / trHeight) : 0; // console.info(
  ...
  $.ui.fancytree._FancytreeClass.prototype._initViewportWrapper = function () {
    ...
    // $wrapper = $(wrapper),
    tree = this; // if (SCROLL_MODE === "scroll") {
    ...
    if (SCROLL_MODE === "wheel") {
      this.$container.on("wheel", function (e) {
        ...
        tree.debug("Got 'wheel' event: dy=" + dy + ", mode=" + orgEvent.deltaMode);
  ...
  $.ui.fancytree._FancytreeClass.prototype._renumberVisibleNodes = function (force, startIdx) {
    if ((!this.options.viewport.enabled || this.visibleNodeList != null) && force !== true) {
    ...
    var i = 0,
        ...
        visibleNodeList = this.visibleNodeList = []; // Reset previous data

    this.visit(function (node) {
      node._rowIdx = null; // node.span = null;
      // if (node.tr) {
    ...
    if (i !== prevLength) {
      this._triggerTreeEvent("updateViewport", null, {
        ...
        next: $.extend({}, this.viewport) // visibleCount: prevLength,
        // cur: i,
  ...
  $.ui.fancytree._FancytreeClass.prototype.redrawViewport = function (force) {
    ...
    this._renumberVisibleNodes(force); // Adjust vp.start value to assure the current content is inside:
    ...
    var i = 0,
        ...
        trIdx = 0,
        trList = this.tbody.children,
        prevPhase = this.isVpUpdating; // Reset previous data
  ...
  $.ui.fancytree.registerExtension({
    ...
    options: {
      ...
      // display 'nodata', 'loading', 'error' centered in a single, merged TR
    ...
    treeInit: function (ctx) {
      ...
      $table.addClass("fancytree-container fancytree-ext-grid fancytree-ext-table");
      ...
      tree.tbody = $tbody[0]; // Prepare row templates:
      // Determine column count from table header if any

      columnCount = $("thead >tr", $table).last().find(">th").length; // Read TR templates from tbody if any
      ...
      $row.find(">td").eq(tableOpts.nodeColumnIdx).html("<span class='fancytree-node' />");
      ...
      this.nodeContainerAttrName = "tr"; // #489: make sure $container is set to <table>, even if ext-dnd is listed before ext-grid
      ...
      this.setViewport($.extend({
        ...
        keepEmptyRows: true,
        noEvents: true
      }, opts.viewport)); // tree.$scrollbar = _addScrollbar($table);

      this._superApply(arguments); // standard Fancytree created a root UL


      $(tree.rootNode.ul).remove();
      tree.rootNode.ul = null; // Add container to the TAB chain
    ...
    nodeRender: function (ctx, force, deep, collapsed, _recursive) {
      ...
      _assert(viewport); // node.debug("nodeRender(): " + node + ", isRoot=" + isRootNode, "tr=" + node.tr, "hcp=" + ctx.hasCollapsedParents, "parent.tr=" + (node.parent && node.parent.tr));
      ...
      if (!_recursive) {
        ...
        ctx.hasCollapsedParents = node.parent && !node.parent.expanded; // Make sure visible row indices are up-to-date
      ...
      if (!isRootNode) {
        outsideViewport = viewport && (node._rowIdx < start || node._rowIdx >= start + viewport.count); // node.debug(
        // 	"nodeRender(): idx=" +
        // 		node._rowIdx +
        // 		", outside=" +
        // 		outsideViewport +
    ...
    nodeRenderTitle: function (ctx, title) {
      ...
      if (isStatusNode) {
        if (opts.renderStatusColumns) {
          ...
          opts.renderStatusColumns.call(tree, {
            type: "renderStatusColumns"
          }, ctx);
        } else if (opts.grid.mergeStatusColumns && node.isTopLevel()) {
          node.warn("mergeStatusColumns is not yet implemented."); // This approach would not work, since the roe may be re-used:
          // $(node.tr)
          // 	.find(">td")
          // 	.eq(0)
    ...
    nodeRenderStatus: function (ctx) {
      ...
      $(node.tr).removeClass("fancytree-node"); // indent
    ...
    nodeSetExpanded: function (ctx, flag, callOpts) {
      var node = ctx.node,
          tree = ctx.tree; // flag defaults to true
      ...
      function _afterExpand(ok) {
        ...
        if (ok) {
          if (flag && ctx.options.autoScroll && !callOpts.noAnimation && node.hasChildren()) {
            ...
            node.getLastChild().scrollIntoView(true, {
              topNode: node
            }).always(function () {
