...
(function (factory) {
  ...
  function setChildRowVisibility(parent, flag) {
    parent.visit(function (node) {
      var tr = node.tr; // currentFlag = node.hide ? false : flag; // fix for ext-filter
  ...
  function findPrevRowNode(node) {
    ...
    if (siblings && siblings.length > 1 && siblings[0] !== node) {
      ...
      _assert(prev.tr); // descend to lowest child (with a <tr> tag)
  ...
  $.ui.fancytree.registerExtension({
    ...
    treeInit: function (ctx) {
      ...
      if (opts.renderStatusColumns) {
        if (opts.renderStatusColumns === true) {
          opts.renderStatusColumns = opts.renderColumns; // } else if( opts.renderStatusColumns === "wide" ) {
      ...
      tree.columnCount = $("thead >tr", $table).last().find(">th", $table).length; // Read TR templates from tbody if any
      ...
      tree.rowFragment.appendChild($row.get(0)); // // If tbody contains a second row, use this as status node template
      ...
      this.nodeContainerAttrName = "tr"; // #489: make sure $container is set to <table>, even if ext-dnd is listed before ext-table
    ...
    nodeRemoveChildMarkup: function (ctx) {
      var node = ctx.node; //		node.debug("nodeRemoveChildMarkup()");
    ...
    nodeRemoveMarkup: function (ctx) {
      var node = ctx.node; //		node.debug("nodeRemoveMarkup()");
    ...
    nodeRender: function (ctx, force, deep, collapsed, _recursive) {
      ...
      if (!isRootNode) {
        ...
        if (node.tr) {
          ...
          if (prevNode.tr) {
            ...
            _assert(!prevNode.parent, "prev. row must have a tr, or be system root"); // tree.tbody.appendChild(newRow);
          ...
          node.tr.ftnode = node; // if(opts.aria){
      ...
      if (children && !_recursive) {
        ...
        prevTr = node.tr || null;
        firstTr = tree.tbody.firstChild; // Iterate over all descendants
    ...
    nodeRenderTitle: function (ctx, title) {
      ...
      if (isStatusNode) {
        if (opts.renderStatusColumns) {
          ...
          $(node.tr).find(">td").eq(0).prop("colspan", tree.columnCount).text(node.title).addClass("fancytree-status-merged").nextAll().remove();
      ...
      function _afterExpand(ok) {
        ...
        if (ok) {
          if (flag && ctx.options.autoScroll && !callOpts.noAnimation && ctx.node.hasChildren()) {
            ...
            ctx.node.getLastChild().scrollIntoView(true, {
              topNode: ctx.node
            }).always(function () {
