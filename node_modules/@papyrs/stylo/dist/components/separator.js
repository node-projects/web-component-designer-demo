import { HTMLElement, h, proxyCustomElement } from '@stencil/core/internal/client';

const separatorCss = ":host{--light:#f4f5f8;--light-contrast:#000000;--medium:#6e6d6f;--medium-contrast:#000000;--black:#000000;--black-contrast:#ffffff;--white:#ffffff;--white-contrast:#000000;--highlight:#3880ff;--highlight-rgb:56, 128, 255;--highlight-contrast:#ffffff}div.separator{display:inline-block;vertical-align:middle;width:1px;min-width:1px;margin:0 6px;height:24px;background:var(--stylo-toolbar-separator-background, var(--light))}";

let Separator = class extends HTMLElement {
  constructor() {
    super();
    this.__registerHost();
    this.__attachShadow();
  }
  render() {
    return h("div", { class: "separator" });
  }
  static get style() { return separatorCss; }
};
Separator = /*@__PURE__*/ proxyCustomElement(Separator, [1, "stylo-toolbar-separator"]);
function defineCustomElement() {
  if (typeof customElements === "undefined") {
    return;
  }
  const components = ["stylo-toolbar-separator"];
  components.forEach(tagName => { switch (tagName) {
    case "stylo-toolbar-separator":
      if (!customElements.get(tagName)) {
        customElements.define(tagName, Separator);
      }
      break;
  } });
}
defineCustomElement();

export { Separator as S, defineCustomElement as d };
