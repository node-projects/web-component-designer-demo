...
export class AlignActions {
  ...
  render() {
    return (h(Host, null,
      h("stylo-toolbar-button", { label: i18n.state.toolbar.list_ol, onAction: ($event) => this.toggleList($event.detail, 'ol'), class: this.list === ToolbarList.ORDERED ? 'active' : undefined },
        h(IconOl, null)),
      h("stylo-toolbar-button", { label: i18n.state.toolbar.list_ul, onAction: ($event) => this.toggleList($event.detail, 'ul'), class: this.list === ToolbarList.UNORDERED ? 'active' : undefined },
        h(IconUl, null))));
  }
