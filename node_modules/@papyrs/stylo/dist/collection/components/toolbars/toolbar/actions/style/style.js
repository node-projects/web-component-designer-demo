...
export const Style = ({ align, list, switchToolbarActions, disabledTitle, bold, italic, strikethrough, underline, link, onExecCommand, toggleLink }) => {
  ...
  const renderFontSizeAction = () => {
    ...
    return (h(Fragment, null, h("stylo-toolbar-button", { onAction: () => switchToolbarActions(ToolbarActions.FONT_SIZE), label: i18n.state.toolbar.style_font_size }, h("span", null, "A", h("small", null, "A"))), renderSeparator()));
  };
  const renderColorActions = () => {
    const result = [
      h("stylo-toolbar-button", { onAction: () => switchToolbarActions(ToolbarActions.COLOR), label: i18n.state.toolbar.style_color }, h(IconPalette, null))
    ];
    if (configStore.state.toolbar.style.backgroundColor) {
      result.push(h("stylo-toolbar-button", { onAction: () => switchToolbarActions(ToolbarActions.BACKGROUND_COLOR), label: i18n.state.toolbar.style_background }, h(IconColor, null)));
    }
    return result;
  };
  return (h(Fragment, null, h("stylo-toolbar-text", { disabledTitle: disabledTitle, bold: bold === 'bold', italic: italic === 'italic', underline: underline === 'underline', strikethrough: strikethrough === 'strikethrough', onExecCommand: ($event) => onExecCommand($event) }), renderSeparator(), renderFontSizeAction(), renderColorActions(), renderSeparator(), renderAlignAction(), renderListAction(), renderLinkSeparator(), h("stylo-toolbar-button", { onAction: toggleLink, cssClass: link ? 'active' : undefined, label: i18n.state.toolbar.link }, h(IconLink, null))));
};
