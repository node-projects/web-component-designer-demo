import { Component, Element, Event, Fragment, h, Method, Prop, State, Watch } from '@stencil/core';
...
export class List {
  ...
  render() {
    return (h(Fragment, null,
      this.plugins.map((plugin, i) => this.renderPlugin(plugin, `plugin-${i}`)),
      this.renderEmpty()));
  }
  renderEmpty() {
    ...
    return (h("span", { class: "empty" },
      i18n.state.plugins.no_matches,
      ": ",
      h("strong", null, this.filter)));
  }
  renderPlugin(plugin, key) {
    const { text, icon: iconSrc } = plugin;
    const icon = renderIcon(iconSrc);
    return (h("button", { key: key, onClick: ($event) => this.emitPlugin($event, plugin) },
      icon === undefined && (h("div", Object.assign({ class: "icon" }, (icon === undefined && { innerHTML: iconSrc })))),
      icon,
  ...
  renderText(text) {
    ...
    if (this.filter.length > 0) {
      ...
      return (h(Fragment, null, (_c = split[0]) !== null && _c !== void 0 ? _c : '',
        h("strong", null, (_d = filter[0]) !== null && _d !== void 0 ? _d : ''), (_e = split[1]) !== null && _e !== void 0 ? _e : ''));
    }
    return textValue;
  }
