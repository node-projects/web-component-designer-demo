...
import { defaultState, render, rendered, setState, state } from "./internal.js"; // Elements listening for changes in prefers-color-scheme.
  ...
  return class dark extends Base {
    ...
    [rendered](changed) {
      ...
      if (changed.detectDarkMode) {
        const {
          detectDarkMode
        } = this[state]; // Add/remove element to/from list of elements listening to color
        // scheme.
...
function setDarkModeFromBackgroundColor(element) {
  ...
  if (rgb) {
    const hsl = rgbToHsl(rgb); // We consider any lightness below 50% to be dark.
...
function findBackgroundColor(element) {
  ...
  const hasColor = backgroundColor !== "transparent" && backgroundColor !== "rgba(0, 0, 0, 0)";
