...
import { defaultState, keydown, raiseChangeEvents, render, rendered, setState, state, stateEffects } from "./internal.js";
...
export default function PopupListMixin(Base) {
  ...
  class PopupList extends Base {
    ...
    [stateEffects](state, changed) {
      const effects = super[stateEffects] ? super[stateEffects](state, changed) : {}; // When opening, reset out notion of whether the user has hovered over an
...
event) {
  ...
  if (opened) {
    ...
    if (target && target instanceof Node && items) {
      ...
      const item = items[hoverIndex]; // If the user is hovering over an enabled item, make it current.
