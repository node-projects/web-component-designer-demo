...
import { defaultState, getItemText, goFirst, goLast, goNext, goPrevious, ids, itemsDelegate, keydown, render, rendered, setState, shadowRoot, state, stateEffects, template } from "./internal.js";
import ListBox from "./ListBox.js";
import PopupListMixin from "./PopupListMixin.js";
import SingleSelectAPIMixin from "./SingleSelectAPIMixin.js";
const Base = CursorAPIMixin(DelegateCursorMixin(DelegateItemsMixin(PopupListMixin(SingleSelectAPIMixin(ComboBox)))));
/**
...
class ListComboBox extends Base {
  ...
  set selectedItemValue(selectedItemValue) {
    ...
    const selectedIndex = items.findIndex(item => item.getAttribute("value") === s);
    this[setState]({
      selectedIndex
    });
  }
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // We want the cursor to track the selection, but we don't always want the
    ...
    if (closingNormally) {
      ...
      const currentItemText = currentItem ? this[getItemText](currentItem) : ""; // See notes on mobile at ComboBox.defaultState.
  ...
  get [template]() {
    const result = super[template]; // Wrap default slot with a list.
