...
import { firstRender, ids, keydown, render, shadowRoot } from "./internal.js"; // Symbols for private data members on an element.
...
function FocusCaptureMixin(Base) {
  class FocusCapture extends Base {
    ...
    /** @type {KeyboardEvent} */
    event) {
      const firstElement = firstFocusableElement(this[shadowRoot]); // We need to check both the document active element (to handle case where
      ...
      if (firstElement) {
        const firstElementIsDocumentActive = document.activeElement && (document.activeElement === firstElement || document.activeElement.contains(firstElement));
        const shadowActiveElement = this[shadowRoot].activeElement;
        const firstElementIsShadowActive = shadowActiveElement && (shadowActiveElement === firstElement || deepContains(shadowActiveElement, firstElement));
        const firstElementIsActive = firstElementIsDocumentActive || firstElementIsShadowActive;
        ...
        if (firstElementIsActive && event.key === "Tab" && event.shiftKey) {
          ...
          this[wrappingFocusKey] = false; // Don't mark the event as handled, since we want it to keep bubbling up.
        }
