...
export default function ContentItemsMixin(Base) {
  return class ContentItems extends Base {
    ...
    [stateEffects](state, changed) {
      const effects = super[stateEffects] ? super[stateEffects](state, changed) : {}; // Regenerate items when content changes.

      if (changed.content) {
        /** @type {Node[]} */
        const content = state.content;
        const items = content ? Array.prototype.filter.call(content,
        /** @type {Node} */
        item => isSubstantiveElement(item)) : null;
