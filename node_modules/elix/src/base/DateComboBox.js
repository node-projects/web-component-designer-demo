...
import { defaultState, goDown, goLeft, goRight, goUp, ids, keydown, raiseChangeEvents, render, setState, shadowRoot, state, stateEffects, template } from "./internal.js";
...
class DateComboBox extends Base {
  ...
  changed) {
    ...
    if (changed.calendarPartType) {
      /** @type {any} */
      const calendar = this[ids].calendar;
    ...
    if ((changed.daysOfWeekFormat || changed.calendarPartType) && "daysOfWeekFormat" in cast) {
    ...
    if ((changed.monthFormat || changed.calendarPartType) && "monthFormat" in cast) {
      cast.monthFormat = this[state].monthFormat;
    }

    if ((changed.yearFormat || changed.calendarPartType) && "yearFormat" in cast) {
      cast.yearFormat = this[state].yearFormat;
    }
  }

  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // If the date changed while focused, assume user changed date.
    ...
    if (changed.date || changed.dateTimeFormat || changed.focused || changed.opened) {
      const {
        closeResult,
        date,
        datePriority,
      ...
      if (changed.date && !focused || blur && userChangedDate || closing && userChangedDate && !canceled || changed.dateTimeFormat && datePriority) {
        const formattedDate = date && dateTimeFormat ? this.formatDate(date, dateTimeFormat) : ""; // See notes on mobile at ComboBox.defaultState.
    ...
    if (changed.dateTimeFormat || changed.timeBias || changed.value) {
      ...
      if (dateTimeFormat && (changed.value || !datePriority && (changed.dateTimeFormat || changed.timeBias))) {
  ...
  get [template]() {
    const result = super[template]; // Replace default slot with calendar.
