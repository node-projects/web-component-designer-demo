...
import { defaultState, firstRender, ids, keydown, raiseChangeEvents, render, setState, state, stateEffects, template } from "./internal.js";
/**
...
class AlertDialog extends Dialog {
  ...
  event) {
    ...
    if (key) {
      ...
      const choice = this.choices.find(choice => choice[0].toLowerCase() === key);
  ...
  changed) {
    ...
    if (this[firstRender]) {
      this[ids].choiceButtonContainer.addEventListener("click", async event => {
    ...
    if (changed.choiceButtons) {
      updateChildNodes(this[ids].choiceButtonContainer, this[state].choiceButtons);
    }
  }

  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // When choices or choice button part type changes, regenerate buttons.
  ...
  get [template]() {
    const result = super[template]; // Replace the default slot with a new default slot and a button container.
