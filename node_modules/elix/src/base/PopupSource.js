...
import { defaultState, firstRender, ids, inputDelegate, raiseChangeEvents, render, rendered, setState, shadowRoot, state, stateEffects, template } from "./internal.js";
import LanguageDirectionMixin from "./LanguageDirectionMixin.js";
import layoutPopup from "./layoutPopup.js";
import OpenCloseMixin from "./OpenCloseMixin.js";
import Popup from "./Popup.js";
const resizeListenerKey = Symbol("resizeListener");
const Base = DisabledMixin(FocusVisibleMixin(LanguageDirectionMixin(OpenCloseMixin(ReactiveElement))));
/**
...
class PopupSource extends Base {
  ...
  changed) {
    ...
    if (changed.opened || changed.popupLayout) {
      const {
        opened,
        popupLayout
      } = this[state]; // By default, we reset the styles used to position the popup.
      ...
      if (!opened) {// Popup closed. Use the reset above.
      } else if (!popupLayout) {
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // We reset our popup calculations when the popup closes, or if it's open
...
function choosePopupLayout(element) {
  ...
  const popupRect = element[ids].popup.getBoundingClientRect(); // If the popup defines a frame, ask the frame for its intrinsic size by
