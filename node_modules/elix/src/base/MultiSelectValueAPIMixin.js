...
export default function MultiSelectValueAPIMixin(Base) {
  ...
  class MultiSelectValueAPI extends Base {
    ...
    [stateEffects](state, changed) {
      const effects = super[stateEffects] ? super[stateEffects](state, changed) : {}; // Value tracks the value attribute on the selected item.

      if (changed.selectedItems) {
        const {
          selectedItems
        } = state;
        const selectedValues = selectedItems ? selectedItems.map(item => item.getAttribute("value")) : [];
    ...
    set value(value) {
      ...
      const selectedItemFlags = items.map(item => values.includes(item.getAttribute("value")));
      this[setState]({
        selectedItemFlags
      });
    }
