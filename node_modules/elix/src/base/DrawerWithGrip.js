...
import { defaultState, ids, raiseChangeEvents, render, rendered, scrollTarget, setState, shadowRoot, state, template } from "./internal.js";
/**
...
class DrawerWithGrip extends Drawer {
  ...
  changed) {
    ...
    if (changed.fromEdge || changed.rightToLeft) {
      ...
      this[ids].frame.style.flexDirection = vertical ? "column" : "row"; // Determine what grid we'll use to relatively position the content and
      // the grip.
      ...
      mapFromEdgeToGrid.start = rightToLeft ? mapFromEdgeToGrid.right : mapFromEdgeToGrid.left;
      mapFromEdgeToGrid.end = rightToLeft ? mapFromEdgeToGrid.left : mapFromEdgeToGrid.right; // Determine what cell the grip will go in.
      ...
      mapFromEdgeToGripCell.start = rightToLeft ? mapFromEdgeToGripCell.right : mapFromEdgeToGripCell.left;
      mapFromEdgeToGripCell.end = rightToLeft ? mapFromEdgeToGripCell.left : mapFromEdgeToGripCell.right;
  ...
  get [template]() {
    const result = super[template]; // Replace the default slot with one that includes a grip element.
    //
    ...
    renderParts(result.content, this[state]); // Normally a closed overlay is hidden, but we want to make a closed
