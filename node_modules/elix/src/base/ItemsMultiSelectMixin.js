...
import { defaultState, setState, state, stateEffects, toggleSelectedFlag } from "./internal.js";
/**
...
export default function ItemsMultiSelectMixin(Base) {
  ...
  class ItemsMultiSelect extends Base {
    ...
    [stateEffects](state, changed) {
      ...
      if (changed.selectedItemFlags && state.items && state.selectedItemFlags) {
        const {
          items,
          selectedItemFlags
        } = state; // If new selectedItemFlags array size doesn't match that of items array,
        // create a new selectedItemFlags array that matches size.

        if (selectedItemFlags.length !== items.length) {
          const newSelectedFlags = selectedItemFlags.length > items.length ? // Trim to fit
          selectedItemFlags.slice(0, items.length) : // Stretch to fit
          [...selectedItemFlags, ...Array(items.length - selectedItemFlags.length).fill(false)];
          ...
          const selectedItems = items.filter((item, index) => selectedItemFlags[index]);
          Object.assign(effects, {
            selectedItems
          });
        }
    ...
    [toggleSelectedFlag](index, toggle) {
      ...
      const newSelectedFlags = [...this[state].selectedItemFlags]; // Apply the toggle. If undefined, flip the current value.

      newSelectedFlags[index] = toggle !== undefined ? toggle : !newSelectedFlags[index];
      this[setState]({
        selectedItemFlags: newSelectedFlags
      });
    }
...
export function selectedItemsToFlags(items, selectedItems) {
  return items.map(item => selectedItems ? selectedItems.indexOf(item) >= 0 : false);
}