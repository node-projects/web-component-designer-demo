...
import { defaultState, getItemText, raiseChangeEvents, render, rendered, setState, state, stateEffects } from "./internal.js";
...
class FilterListBox extends ListBox {
  ...
  highlightTextInItem(textToHighlight, item) {
    ...
    const start = textToHighlight ? normalize(text).indexOf(normalize(textToHighlight)) : -1;
  ...
  /** @type {ChangedFlags} */
  changed) {
    super[render](changed); // Hide items that don't match state.
  ...
  [rendered](changed) {
    super[rendered](changed); // If filter changed, we may need to scroll cursor back into view.
  ...
  [stateEffects](state, changed) {
    ...
    if (changed.filter || changed.items) {
      ...
      const availableItemFlags = items === null ? null : items.map(item => this.itemMatchesFilter(item, filter));
