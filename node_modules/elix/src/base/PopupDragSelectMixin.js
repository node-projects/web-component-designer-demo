...
import { defaultState, ids, raiseChangeEvents, rendered, setState, shadowRoot, state, stateEffects } from "./internal.js";
...
export default function PopupDragSelectMixin(Base) {
  ...
  class PopupDragSelect extends Base {
    ...
    [stateEffects](state, changed) {
      const effects = super[stateEffects](state, changed); // Set things when opening, or reset things when closing.
...
event) {
  ...
  if (element.opened) {
    ...
    const overSource = hitTargets.indexOf(element[ids].source) >= 0; // Was mouseup over the popup or popup frame?
