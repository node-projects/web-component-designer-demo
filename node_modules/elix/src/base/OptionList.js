...
import { defaultState, ids, render, scrollTarget, setState, state, template } from "./internal.js";
...
const Base = AriaRoleMixin(ComposedFocusMixin(CursorAPIMixin(CursorInViewMixin(DirectionCursorMixin(FocusVisibleMixin(FormElementMixin(ItemsAPIMixin(ItemsCursorMixin(ItemsTextMixin(KeyboardDirectionMixin(KeyboardMixin(KeyboardPagedCursorMixin(KeyboardPrefixCursorMixin(LanguageDirectionMixin(SingleSelectAPIMixin(SelectedTextAPIMixin(SelectedValueAPIMixin(SlotItemsMixin(TapCursorMixin(ReactiveElement))))))))))))))))))));
/**
 * A list of options.
 *
...
class OptionList extends Base {
  ...
  /** @type {ChangedFlags} */
  changed) {
    super[render](changed); // Highlight the current item.
    ...
    if (changed.items || changed.currentIndex || changed.highlightCurrentItem) {
      ...
      if (items) {
        items.forEach((item, index) => {
          const current = index === currentIndex; // Show visible highlight unless we're surpressing that.

          item.toggleAttribute("current", highlightCurrentItem && current); // For ARIA purposes, we want to announce the current item as the
          // selected item.
