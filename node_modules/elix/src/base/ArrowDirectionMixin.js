...
import { defaultState, goNext, goPrevious, ids, raiseChangeEvents, render, setState, shadowRoot, state } from "./internal.js";
  ...
  class ArrowDirection extends Base {
    ...
    changed) {
      ...
      if (changed.arrowButtonPartType) {
        ...
        arrowButtonPrevious.addEventListener("mousedown", previousButtonHandler);
        const arrowButtonNext = this[ids].arrowButtonNext;
      ...
      const arrowButtonNext = this[ids].arrowButtonNext; // Position the buttons.

      if (changed.arrowButtonOverlap || changed.orientation || changed.rightToLeft) {
        this[ids].arrowDirection.style.flexDirection = vertical ? "column" : "row";
        this[ids].arrowDirectionContainer.style.maxHeight = vertical ? "100%" : "";
        const buttonStyle = {
          bottom: null,
          left: null,
          right: null,
    ...
    [wrap](target) {
      ...
      `;
      renderParts(arrowControls, this[state]); // Wrap the target with the arrow controls.
...
function createButtonHandler(element, callback) {
  return async function mousedown(
  /** @type {Event} */
  event) {
