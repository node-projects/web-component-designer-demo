 ...
 * @module calendar
 */
import weekData from "./weekData.js"; // Default region is "World", see https://www.ctrl.blog/entry/en-001
...
export function parse(text, dateTimeFormat) {
  const today = new Date(); // @ts-ignore

  /** @type {any[]} */

  const parts = dateTimeFormat.formatToParts(today); // Convert parts to a regex.
  ...
  const regExText = parts.map(part => part.type === "literal" ? "(\\D+)" : // TODO: use named capture group `(<${part.type}>\\d+)`
  // when that's widely supported.
  `(\\d+)`).join("");
  const regEx = new RegExp(regExText); // Match against the text.
  ...
  const {
    ...
    second,
    year
  } = groups; // Adjust short year to current century.

  const yearValue = year && parseInt(year);
  const adjustedYear = yearValue < 100 ? 2000 + yearValue : year;
  return new Date(adjustedYear || today.getFullYear(), month !== undefined ? month - 1 : today.getMonth(), day || today.getDate(), hour || 0, minute || 0, second || 0);
}
/**
...
export function parseWithOptionalYear(text, dateTimeFormat, timeBias) {
  ...
  if (abbreviatedDate && timeBias) {
    ...
    if (timeBias === "future") {
      if (abbreviatedMonth < todayMonth || abbreviatedMonth === todayMonth && abbreviatedDay < todayDay) {
