...
import { defaultState, ids, inputDelegate, keydown, raiseChangeEvents, render, rendered, setState, shadowRoot, state, stateEffects, template } from "./internal.js";
...
const Base = DelegateFocusMixin(DelegateInputLabelMixin(DelegateInputSelectionMixin(FocusVisibleMixin(FormElementMixin(KeyboardMixin(PopupDragSelectMixin(PopupToggleMixin(PopupSource))))))));
/**
...
class ComboBox extends Base {
  ...
  changed) {
    ...
    if (changed.popupPartType) {
      ...
      const cast = popup; // Make popup not focusable.

      popup.removeAttribute("tabindex"); // Override popup's backdrop to hide it.
    ...
    if (changed.disabled) {
      ...
      this[ids].input.disabled = disabled;
      /** @type {any} */
    ...
    if (changed.placeholder) {
      ...
      this[ids].input.placeholder = placeholder;
    }
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // If the value changes while the popup is closed (or closing), consider the
  ...
  get [template]() {
    const result = super[template]; // Put an input element and toggle in the source.
