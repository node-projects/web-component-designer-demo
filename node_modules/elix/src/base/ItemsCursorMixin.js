...
import { closestAvailableItemIndex, defaultState, goFirst, goLast, goNext, goPrevious, setState, state, stateEffects } from "./internal.js";
/**
...
export default function ItemsCursorMixin(Base) {
  ...
  class ItemsCursor extends Base {
    ...
    [stateEffects](state, changed) {
      const effects = super[stateEffects] ? super[stateEffects](state, changed) : {}; // Ensure currentIndex is valid.
      ...
      if (changed.availableItemFlags || changed.items || changed.currentIndex || changed.currentItemRequired) {
        ...
        const count = items ? items.length : 0; // Determine the desired index: the one we want irrespective of whether
        ...
        if (newDesiredIndex < 0) {
          ...
          newIndex = Math.max(Math.min(count - 1, newDesiredIndex), 0); // Look for an available item going forward.
