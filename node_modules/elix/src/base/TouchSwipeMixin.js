...
import { defaultState, firstRender, raiseChangeEvents, render, setState, state, stateEffects, swipeDown, swipeLeft, swipeRight, swipeStart, swipeTarget, swipeUp } from "./internal.js";
...
function gestureContinue(element, clientX, clientY, eventTarget) {
  /** @type {any} */
  const cast = element; // Calculate and save the velocity since the last event. If this is the last
  ...
  cast[previousVelocityKey] = velocity; // Was this specific event more vertical or more horizontal?

  const eventAxis = Math.abs(deltaY) > Math.abs(deltaX) ? "vertical" : "horizontal"; // Is this the first touch move event in a swipe sequence?
  ...
  if (cast[deferToScrollingKey]) {
    ...
    const willScroll = canScrollInDirection(eventTarget, swipeAxis, downOrRight);
  ...
  const swipeFraction = Math.max(Math.min(fraction, swipeFractionMax), swipeFractionMin);
...
function gestureEnd(element, clientX, clientY, eventTarget) {
  ...
  const vertical = swipeAxis === "vertical"; // Scrolling takes precedence over flick gestures.
  ...
  if (!willScroll) {
    ...
    if (typeof flickPositive !== "undefined") {
      const gesture = vertical ? flickPositive ? swipeDown : swipeUp : flickPositive ? swipeRight : swipeLeft; // If component has method for indicated gesture, invoke it.
...
function getSwipeFraction(element, x, y) {
  ...
  const swipeTargetSize = vertical ? element[swipeTarget].offsetHeight : element[swipeTarget].offsetWidth;
