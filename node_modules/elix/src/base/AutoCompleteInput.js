...
import { defaultState, firstRender, ids, inputDelegate, matchText, raiseChangeEvents, render, rendered, setState, shadowRoot, state, template } from "./internal.js";
...
class AutoCompleteInput extends Input {
  ...
  [matchText](texts, prefix) {
    ...
    const match = texts.find(text => text.toLowerCase().startsWith(prefixLowerCase));
    return match || null;
  }
  ...
  changed) {
    ...
    if (this[firstRender]) {
      ...
      this[ids].inner.addEventListener("input", () => {
        ...
        setTimeout(() => {
          ...
          const text = this.value.toLowerCase(); // We only AutoComplete if the user's typing at the end of the input.
          // Read the selection start and end directly off the inner element to
          // ensure they're up to date.

          const typingAtEnd = inner.selectionStart === text.length && inner.selectionEnd === text.length; // Moreover, we only AutoComplete if we're sure the user's added a
          ...
          const userAddedText = text.startsWith(originalText) && text.length === originalText.length + 1;
    ...
    if (changed.texts) {
      ...
      const options = texts === null ? [] : texts.map(text => {
  ...
  get [template]() {
    const result = super[template]; // Apply ARIA combobox attributes to the input.
  ...
  set value(value) {
    super.value = value; // If the input has focus, we assume the user is typing, and rely on
...
element) {
  const match = element[matchText](element.texts, element.value); // If found, update the input value to the match.
