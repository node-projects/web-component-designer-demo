...
import { defaultState, keydown, render, rendered, setState, state, template } from "./internal.js";
...
const Base = AriaListMixin(CursorAPIMixin(CursorSelectMixin(DirectionCursorMixin(ItemsAPIMixin(ItemsCursorMixin(KeyboardDirectionMixin(KeyboardMixin(LanguageDirectionMixin(SingleSelectAPIMixin(SlotItemsMixin(TapCursorMixin(ReactiveElement))))))))))));
/**
...
class TabStrip extends Base {
  ...
  /** @type {KeyboardEvent} */
  event) {
    let handled; // Let user select a tab button with Enter or Space.
  ...
  changed) {
    ...
    if (changed.orientation) {
      this.style.gridAutoFlow = this[state].orientation === "vertical" ? "row" : "column";
    }
  ...
  /** @type {ChangedFlags} */
  changed) {
    super[rendered](changed); // Does this component, or any of its assigned nodes, have focus?
    ...
    if (focused && selectedItem && selectedItem instanceof HTMLElement && selectedItem !== document.activeElement) {
      selectedItem.focus();
    }
  }
  /**
