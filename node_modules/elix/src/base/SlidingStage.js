...
const Base = CursorAPIMixin(CursorSelectMixin(EffectMixin(ItemsAPIMixin(ItemsCursorMixin(LanguageDirectionMixin(SingleSelectAPIMixin(SlotItemsMixin(ReactiveElement))))))));
/**
...
class SlidingStage extends Base {
  ...
  /** @type {ChangedFlags} */
  changed) {
    super[render](changed); // Apply `selected` attribute to the selected item only.
    ...
    if (changed.currentIndex || changed.enableEffects || changed.orientation || changed.swipeFraction) {
      const {
        orientation,
        rightToLeft,
        currentIndex,
      ...
      slidingStageContent.style.transition = showTransition ? "transform 0.25s" : "none";
    }
    ...
    if (changed.orientation) {
      ...
      const vertical = orientation === "vertical";
      this[ids].slidingStageContent.style.flexDirection = vertical ? "column" : "";
    }
  }
