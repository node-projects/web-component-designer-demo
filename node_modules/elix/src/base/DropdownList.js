...
import { applyElementData, defaultState, ids, render, rendered, setState, shadowRoot, state, stateEffects, template } from "./internal.js";
...
const Base = CursorAPIMixin(DelegateInputLabelMixin(FormElementMixin(ItemsAPIMixin(ItemsCursorMixin(PopupListMixin(SelectedTextAPIMixin(SelectedValueAPIMixin(SingleSelectAPIMixin(SlotItemsMixin(PopupButton))))))))));
/**
...
class DropdownList extends Base {
  ...
  changed) {
    ...
    renderParts(this[shadowRoot], this[state], changed); // Update selection.
    ...
    if (changed.items || changed.selectedIndex) {
      ...
      const selectedItem = items ? items[selectedIndex] : null; // Apply the data from the selected item to the value part.

      this[applyElementData](selectedItem, this[ids].value); // Mark only the selected item as selected.
    ...
    if (changed.sourcePartType) {
      /** @type {any} */
      const source = this[ids].source; // HACK — need better way to forward button role to inner button.
  ...
  [rendered](changed) {
    super[rendered](changed); // Indicate which component is the popup's list.
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // When opening the popup, by default (re)select the current item.
  ...
  get [template]() {
    const result = super[template]; // Replace the source slot with an element to show the value.

    const sourceSlot = result.content.querySelector('slot[name="source"]');

    if (sourceSlot) {
      replace(sourceSlot, fragmentFrom.html` <div id="value" part="value"></div> `);
