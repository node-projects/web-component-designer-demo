...
import { defaultState, effectEndTarget, ids, raiseChangeEvents, render, rendered, scrollTarget, setState, state, stateEffects, swipeDown, swipeLeft, swipeRight, swipeTarget, swipeUp, template } from "./internal.js";
...
const Base = DialogModalityMixin(EffectMixin(FocusCaptureMixin(KeyboardMixin(LanguageDirectionMixin(TouchSwipeMixin(TrackpadSwipeMixin(TransitionEffectMixin(Overlay))))))));
/**
...
class Drawer extends Base {
  ...
  changed) {
    ...
    if (changed.gripSize || changed.opened || changed.swipeFraction) {
      ...
      const openedOrSwiping = opened || swiping; // Only listen to pointer events if opened or swiping.

      this.style.pointerEvents = openedOrSwiping ? "initial" : "none"; // Clip frame to its bounding box when drawer is completely closed. This
    ...
    if (changed.effect || changed.effectPhase || changed.fromEdge || changed.gripSize || changed.openedFraction || changed.rightToLeft || changed.swipeFraction) {
      const {
      ...
      const fromLeadingEdge = fromEdge === "left" || fromEdge === "top" || fromEdge === "start" && !rightToLeft || fromEdge === "end" && rightToLeft;
      const swiping = swipeFraction !== null;
      ...
      const transitionFraction = Math.abs(openedFraction - openedRenderedFraction);
      const duration = showTransition ? transitionFraction * (drawerTransitionDuration / 1000) : 0;
      ...
      const translateValue = gripValue === 0 ? translatePercentage : `calc(${translatePercentage} + ${gripValue}px)`;
    ...
    if (changed.fromEdge || changed.rightToLeft) {
      const {
        fromEdge,
        rightToLeft
      } = this[state]; // Stick drawer to all edges except the one opposite the fromEdge.
      ...
      mapFromEdgeToOppositeEdge.start = mapFromEdgeToOppositeEdge[rightToLeft ? "right" : "left"];
      mapFromEdgeToOppositeEdge.end = mapFromEdgeToOppositeEdge[rightToLeft ? "left" : "right"];
      ...
      this.style.flexDirection = fromEdge === "top" || fromEdge === "bottom" ? "column" : "row";
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects] ? super[stateEffects](state, changed) : {}; // Have swipeAxis follow fromEdge.

    if (changed.fromEdge) {
      const {
        fromEdge
      } = state;
      const swipeAxis = fromEdge === "top" || fromEdge === "bottom" ? "vertical" : "horizontal";
    ...
    if (changed.effect || changed.effectPhase || changed.fromEdge || changed.rightToLeft || changed.swipeFraction) {
      const {
        effect,
        effectPhase,
        fromEdge,
      ...
      const opened = effect === "open" && effectPhase !== "before" || effect === "close" && effectPhase === "before";
      const fromLeadingEdge = fromEdge === "left" || fromEdge === "top" || fromEdge === "start" && !rightToLeft || fromEdge === "end" && rightToLeft; // Constrain the opened fraction to between 0 and a bit less than 1. A
      ...
      const almost1 = 0.999; // Swipe bounds depend on whether drawer is current open or closed.

      const expectPositiveSwipe = fromLeadingEdge && !opened || !fromLeadingEdge && opened;
      ...
      if (swiping) {
        const boundedSwipeFraction = Math.max(Math.min(swipeFraction, upperBound), lowerBound);
    ...
    if (changed.enableEffects || changed.effect || changed.effectPhase || changed.swipeFraction) {
      const {
        enableEffects,
        effect,
        effectPhase,
      ...
      const swiping = swipeFraction !== null;
      const showTransition = enableEffects && !swiping && effect && (effectPhase === "during" || effectPhase === "after");
      Object.assign(effects, {
        showTransition
      });
    }
  ...
  async [swipeLeft]() {
    ...
    const fromLeftEdge = fromEdge === "left" || fromEdge === "start" && !rightToLeft || fromEdge === "end" && rightToLeft;
    const fromRightEdge = fromEdge === "right" || fromEdge === "start" && rightToLeft || fromEdge === "end" && !rightToLeft;
  ...
  get [template]() {
    const result = super[template]; // Wrap default slot with another container that traps focus.
