...
import { defaultState, firstRender, ids, raiseChangeEvents, render, rendered, scrollTarget, setState, state, stateEffects, tap, template } from "./internal.js";
...
const Base = AriaMenuMixin(CursorAPIMixin(CursorInViewMixin(DelegateFocusMixin(DirectionCursorMixin(ItemsAPIMixin(ItemsCursorMixin(ItemsTextMixin(KeyboardDirectionMixin(KeyboardMixin(KeyboardPagedCursorMixin(KeyboardPrefixCursorMixin(LanguageDirectionMixin(SlotItemsMixin(TapCursorMixin(ReactiveElement)))))))))))))));
/**
...
class Menu extends Base {
  ...
  changed) {
    ...
    const {
      currentIndex,
      items
    } = this[state]; // Highlight the current item.
  ...
  changed) {
    ...
    if (!this[firstRender] && changed.currentIndex && !this[state].currentItemFocused) {
      ...
      focusElement.focus(); // Now that the current item has been focused, we can remove/reset the
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // When current item changes, we'll need to focus on it in rendered.
