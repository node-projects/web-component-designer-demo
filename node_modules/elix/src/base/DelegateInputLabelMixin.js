...
import { defaultState, firstRender, inputDelegate, raiseChangeEvents, render, rendered, setState, state, stateEffects } from "./internal.js";
/**
...
export default function DelegateInputLabelMixin(Base) {
  ...
  class DelegateInputLabel extends Base {
    ...
    [stateEffects](state, changed) {
      const effects = super[stateEffects] ? super[stateEffects](state, changed) : {}; // If the ariaLabel changes, we can update our inputLabel state
...
function refreshInputLabel(element, state) {
  ...
  let inputLabel = null; // Prefer aria-labelledby over aria-label, per
  ...
  if (ariaLabelledby && rootNode) {
    ...
    const labels = ids.map(id => {
      const elementWithId = rootNode.getElementById(id); // Get a label from the indicated element.
