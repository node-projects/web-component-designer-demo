...
import { defaultState, firstRender, render, setState, state, stateEffects, template } from "./internal.js";
import ItemsAPIMixin from "./ItemsAPIMixin.js";
import ItemsCursorMixin from "./ItemsCursorMixin.js";
import SingleSelectAPIMixin from "./SingleSelectAPIMixin.js";
import SlotItemsMixin from "./SlotItemsMixin.js";
import TransitionEffectMixin from "./TransitionEffectMixin.js";
const Base = CursorAPIMixin(CursorSelectMixin(EffectMixin(ItemsAPIMixin(ItemsCursorMixin(SingleSelectAPIMixin(SlotItemsMixin(TransitionEffectMixin(ReactiveElement))))))));
/**
...
class CrossfadeStage extends Base {
  ...
  changed) {
    ...
    if (changed.effect || changed.effectPhase || changed.enableEffects || changed.rightToLeft || changed.items || changed.currentIndex || changed.swipeFraction || changed.transitionDuration) {
      ...
      if (items && effect === "select") {
        if (enableEffects && effectPhase === "before") {
        ...
        } else if (enableEffects && effectPhase === "during" || swipeFraction != null) {
          ...
          items.forEach((item, index) => {
            const opacity = opacityForItemWithIndex(index, currentIndex, selectionFraction);
    ...
    if (changed.enableEffects || changed.items || changed.swipeFraction || changed.transitionDuration) {
      ...
      const transition = enableEffects && swipeFraction == null ? `opacity ${transitionDuration / 1000}s linear` : "";
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // When selection changes, (re)start the selection effect.

    if (changed.currentIndex) {
      const effectPhase = state.enableEffects && state.currentIndex >= 0 && state.effectPhase !== "before" ? "before" : "after"; // We'll watch the selected item to see when its `transitionend` event
      // fires; that will signal the end of the effect.

      const effectEndTarget = state.items && state.items[state.currentIndex] ? state.items[state.currentIndex] : null;
