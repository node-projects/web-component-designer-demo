...
function getPositionedRect(sourceRect, popupRect, boundsRect, direction, align) {
  ...
  const sourceOrigin = getSourceOrigin(sourceRect, direction, align); // We'll adjust our bounds depending upon the layout.
...
export default function layoutPopup(sourceRect, popupRect, boundsRect, options) {
  const normalized = normalizeOptions(options); // Given the direction and alignment, which layouts do we want to consider?

  const layouts = prioritizedLayouts(normalized.direction, normalized.align); // Sort layouts by our heuristic.

  layouts.sort((layout1, layout2) => compareLayouts(layout1, layout2, sourceRect, popupRect, boundsRect)); // Take the best layout.

  const layout = layouts[0]; // Find the positioned rect with respect to the source origin.
