...
import { defaultState, firstRender, ids, raiseChangeEvents, render, rendered, setState, shadowRoot, state, stateEffects, swipeTarget, template } from "./internal.js";
...
class PullToRefresh extends Base {
  ...
  changed) {
    ...
    if (this[firstRender]) {
      ...
      scrollTarget.addEventListener("scroll", async () => {
        ...
        this[raiseChangeEvents] = true; // Desktop and Mobile Safari don't agree on how to expose document
        ...
        const scrollTop = scrollTarget instanceof Window ? window.pageYOffset : scrollTarget.scrollTop;
    ...
    if (changed.refreshing) {
      ...
      if ("playing" in this[ids].refreshingIndicator) {
        /** @type {any} */
        refreshingIndicator.playing = refreshing;
      }
    }
    ...
    if (changed.pullTriggeredRefresh || changed.refreshing || changed.scrollPullDistance || changed.swipeFraction) {
      const {
      ...
      const showPullIndicator = !refreshing && !pullTriggeredRefresh && pullingDown;
      this[ids].pullIndicator.style.visibility = showPullIndicator ? "visible" : "hidden";
    }
  }
  ...
  changed) {
    ...
    if (this[state].swipeFraction > 0 && !this[state].refreshing && !this[state].pullTriggeredRefresh) {
  ...
  set refreshingIndicatorPartType(refreshingIndicatorPartType) {
    this[setState]({
      refreshingIndicatorPartType
    });
  }

  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // We use a pullTriggeredRefresh flag to track whether the current pull
...
function getSwipeThreshold(element) {
  ...
  return indicatorContainer instanceof HTMLElement ? indicatorContainer.offsetHeight : 0;
}
/**
...
function getTranslationForSwipeFraction(state, swipeTarget) {
  ...
  } = state; // When damping, we halve the swipe fraction so the user has to drag twice as
  // far to get the usual damping. This produces the feel of a tighter, less
  // elastic surface.

  let result = swipeFraction ? swipeTarget.offsetHeight * dampen(swipeFraction / 2) : 0;
...
async function handleScrollPull(element, scrollTop) {
  if (scrollTop < 0) {
    ...
    if (element[state].scrollPullDistance && !element[state].scrollPullMaxReached && scrollPullDistance < element[state].scrollPullDistance) {
