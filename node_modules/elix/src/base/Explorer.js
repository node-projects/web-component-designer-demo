...
import { checkSize, closestAvailableItemIndex, defaultState, firstRender, ids, raiseChangeEvents, render, setState, shadowRoot, state, stateEffects, template } from "./internal.js";
...
import SlotItemsMixin from "./SlotItemsMixin.js"; // Does a list position imply a lateral arrangement of list and stage?
...
const Base = CursorAPIMixin(CursorSelectMixin(ItemsAPIMixin(ItemsCursorMixin(LanguageDirectionMixin(SingleSelectAPIMixin(SlotItemsMixin(ReactiveElement)))))));
/**
...
class Explorer extends Base {
  ...
  changed) {
    ...
    if (this[firstRender]) {
      ...
      this[ids].proxySlot.addEventListener("slotchange", () => {
        ...
        /** @type {any} */
        this[ids].proxySlot;
        const proxies = proxySlot.assignedNodes({
          flatten: true
        });
    ...
    const handleSelectedIndexChanged =
    /** @type {CustomEvent} */
    event => {
    ...
    if (changed.proxyListPartType) {
      this[ids].proxyList.addEventListener("selectedindexchange", handleSelectedIndexChanged);
    }

    if (changed.stagePartType) {
      this[ids].stage.addEventListener("selectedindexchange", handleSelectedIndexChanged);
    ...
    if (changed.proxies || changed.proxiesAssigned) {
      ...
      const childNodes = proxiesAssigned ? [this[ids].proxySlot] : [this[ids].proxySlot, ...proxies];
      updateChildNodes(this[ids].proxyList, childNodes);
    }

    if (changed.proxyListOverlap || changed.proxyListPosition || changed.proxyListPartType) {
      const {
        proxyListOverlap,
        proxyListPosition
      } = this[state];
    ...
    if (changed.proxyListPosition || changed.proxyListPartType) {
      ...
      const lateralPosition = lateralPositions[proxyListPosition];
      this[ids].explorerContainer.style.flexDirection = lateralPosition ? "row" : "column";
      Object.assign(proxyList.style, {
    ...
    if (changed.currentIndex || changed.proxyListPartType) {
      if ("selectedIndex" in proxyList) {
        ...
        proxyList.selectedIndex = currentIndex;
      }
    }
    ...
    if (changed.currentIndex || changed.stagePartType) {
      if ("selectedIndex" in stage) {
        ...
        stage.selectedIndex = currentIndex;
      }
    }
    ...
    if (changed.currentItemRequired || changed.proxyListPartType) {
      if ("selectionRequired" in proxyList) {
        ...
        proxyList.selectionRequired = selectionRequired;
      }
    }
    ...
    if (changed.swipeFraction || changed.proxyListPartType) {
      if ("swipeFraction" in proxyList) {
        ...
        proxyList.swipeFraction = swipeFraction;
      }
    }
    ...
    if (changed.swipeFraction || changed.stagePartType) {
      if ("swipeFraction" in stage) {
        ...
        stage.swipeFraction = swipeFraction;
      }
    }
  }
  /**
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // If items for default proxies have changed, recreate the proxies.
    ...
    if (changed.currentIndex || changed.cursorOperationsWrap || changed.filter || changed.items) {
      const {
        currentIndex,
        items
      } = state; // Can go next/previous if there are items but no cursor.

      const specialCase = items && items.length > 0 && currentIndex < 0;
      const canGoNext = specialCase || this[closestAvailableItemIndex](state, {
        direction: 1,
        index: currentIndex + 1
      }) >= 0;
      const canGoPrevious = specialCase || this[closestAvailableItemIndex](state, {
        direction: -1,
        index: currentIndex - 1
      }) >= 0;
      Object.assign(effects, {
...
function findChildContainingNode(root, node) {
  ...
  return parentNode === root ? node : parentNode ? findChildContainingNode(root, parentNode) : null;
}
/**
...
function setListAndStageOrder(element, state) {
  ...
  const listInInitialPosition = proxyListPosition === "top" || proxyListPosition === "start" || proxyListPosition === "left" && !rightToLeft || proxyListPosition === "right" && rightToLeft;
