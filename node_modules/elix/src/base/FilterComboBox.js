...
import { defaultState, ids, raiseChangeEvents, render, setState, state, stateEffects } from "./internal.js";
...
class FilterComboBox extends Base {
  ...
  changed) {
    ...
    if (changed.inputPartType) {
      this[ids].input.addEventListener("input", event => {
        ...
        const cast = event;
        const filter = cast.detail ? cast.detail.originalText : this[state].value;
        this[setState]({
          filter
        });
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // Closing resets the filter.
