...
import { defaultState, defaultTabIndex, firstRender, ids, nativeInternals, render, rendered, setState, shadowRoot, state, template } from "./internal.js";
...
const mouseEventNames = ["click", "dblclick", "mousedown", "mouseenter", "mouseleave", "mousemove", "mouseout", "mouseover", "mouseup", "wheel"]; // Keep track of which re-raised events should bubble.
...
const blockElements = ["address", "article", "aside", "blockquote", "canvas", "dd", "div", "dl", "fieldset", "figcaption", "figure", "footer", "form", "h1", "h2", "h3", "h4", "h5", "h6", "header", "hgroup", "hr", "li", "main", "nav", "noscript", "ol", "output", "p", "pre", "section", "table", "tfoot", "ul", "video"]; // Standard attributes that don't have corresponding properties.
...
class WrappedStandardElement extends Base {
  ...
  changed) {
    ...
    this.constructor[delegatedPropertySettersKey].forEach(property => {
      if (changed[property]) {
        const value = this[state][property]; // Inner selection properties needed to be handled specially.
  ...
  /** @type {ChangedFlags} */
  changed) {
    super[rendered](changed); // Apply disabled state.
  ...
  static wrap(extendsTag) {
    ...
    class Wrapped extends WrappedStandardElement {} // Indicate which tag it wraps.

    /** @type {any} */


    Wrapped[extendsKey] = extendsTag; // Create getter/setters that delegate to the wrapped element.
