...
import { defaultState, ids, render, setState, state, stateEffects, template } from "./internal.js";
...
class CalendarDays extends Base {
  ...
  changed) {
    ...
    if (changed.date || changed.locale || changed.showSelectedDay) {
      ...
      days.forEach(day => {
        ...
        /** @type {any} */
        day.date;
        const selected = showSelectedDay && dayDate.getDate() === selectedDate && dayDate.getMonth() === selectedMonth && dayDate.getFullYear() === selectedYear;
    ...
    if (changed.dayCount || changed.startDate) {
      ...
      days.forEach(day => {
        if ("outsideRange" in day) {
          ...
          const outsideRange = dayTime < startDate.getTime() || dayTime >= firstDateAfterRange.getTime();
  ...
  [stateEffects](state, changed) {
    const effects = super[stateEffects](state, changed); // If any date-related state changes, regenerate the set of days.

    if (changed.dayCount || changed.dayPartType || changed.locale || changed.showCompleteWeeks || changed.startDate) {
...
function updateDays(state, forceCreation) {
  ...
  const workingStartDate = showCompleteWeeks ? calendar.firstDateOfWeek(startDate, locale) : calendar.midnightOnDate(startDate);
  let workingDayCount;
