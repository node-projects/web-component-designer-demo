...
import { defaultState, ids, render, setState, state, template } from "./internal.js";
...
const Base = CursorAPIMixin(CursorSelectMixin(EffectMixin(ItemsAPIMixin(ItemsCursorMixin(LanguageDirectionMixin(ResizeMixin(SingleSelectAPIMixin(SlotItemsMixin(TapCursorMixin(ReactiveElement))))))))));
/**
...
class CenteredStrip extends Base {
  ...
  changed) {
    ...
    if (changed.clientWidth || changed.enableEffects || changed.rightToLeft || changed.currentIndex || changed.swipeFraction) {
      const {
        orientation,
        rightToLeft,
        currentIndex
      ...
      const dimension = vertical ? "offsetHeight" : "offsetWidth"; // @ts-ignore

      const stripContainerDimension = this[ids].stripContainer[dimension]; // @ts-ignore

      const stripDimension = this[ids].strip[dimension]; // It seems this method can be invoked before the strip any height/width.
      ...
      if (stripDimension > 0) {
        ...
        if (stripDimension <= stripContainerDimension) {
          ...
          const itemBeforeCenter = itemBefore instanceof HTMLElement ? itemBefore[leadingEdge] + itemBefore[dimension] / 2 : 0;
          const itemAfterIndex = itemBeforeIndex + 1;
          const itemAfter = this.items && this.items[itemAfterIndex];
          const itemAfterCenter = itemAfter instanceof HTMLElement ? itemAfter[leadingEdge] + itemAfter[dimension] / 2 : 0;
          let center = 0;
          ...
          if (itemBefore && !itemAfter) {
            ...
            const offsetFraction = selectionFraction - itemBeforeIndex; // TODO: sign
          ...
          translation = center - stripContainerDimension / 2; // Constrain x to avoid showing space on either end.
    ...
    if (changed.orientation) {
      const flexDirection = this[state].orientation === "horizontal" ? "" : "column";
