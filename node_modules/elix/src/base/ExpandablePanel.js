...
import { defaultState, effectEndTarget, ids, render, setState, state, template } from "./internal.js";
import OpenCloseMixin from "./OpenCloseMixin.js";
import TransitionEffectMixin from "./TransitionEffectMixin.js";
const Base = OpenCloseMixin(EffectMixin(TransitionEffectMixin(ReactiveElement)));
/**
  ...
  changed) {
    ...
    if (changed.effect || changed.effectPhase || changed.enableEffects) {
      const {
        ...
        enableEffects,
        transitionDuration
      } = this[state]; // The inner container lets us measure how tall the content wants to be.

      const naturalHeight = this[ids].innerContainer.getBoundingClientRect().height; // The effect phase (before, during, after) determines which height we apply
      ...
      const height = phaseHeights[effect][effectPhase]; // This animates an element's height, which may not produce the smoothest
      // results. See
      ...
      const transition = enableEffects && effectPhase === "during" ? `height ${durationInSeconds}s` : null;
