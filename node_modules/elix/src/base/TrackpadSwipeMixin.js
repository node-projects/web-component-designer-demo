...
import { defaultState, raiseChangeEvents, scrollTarget, setState, state, stateEffects, swipeDown, swipeLeft, swipeRight, swipeStart, swipeTarget, swipeUp } from "./internal.js";
...
const wheelSequenceAxisKey = Symbol("wheelSequenceAxis"); // Time we wait following a gesture before paying attention to wheel events
// again.

const POST_GESTURE_TIME = 250; // Time we wait after the last wheel event before we reset things.
...
export default function TrackpadSwipeMixin(Base) {
  ...
  return class TrackpadSwipe extends Base {
    ...
    [stateEffects](state, changed) {
      const effects = super[stateEffects] ? super[stateEffects](state, changed) : {}; // If the swipeFraction crosses the -0.5 or 0.5 mark, update our notion of
...
function handleWheel(element, event) {
  /** @type {any} */
  const cast = element; // Since we have a new wheel event, reset our timer waiting for the last
  // wheel event to pass.
  ...
  const deltaY = event.deltaY; // See if component event represents acceleration or deceleration.
  ...
  const acceleration = vertical ? Math.sign(deltaY) * (deltaY - cast[lastDeltaYKey]) : Math.sign(deltaX) * (deltaX - cast[lastDeltaXKey]);
  cast[lastDeltaXKey] = deltaX;
  cast[lastDeltaYKey] = deltaY; // Is this the first wheel event in a swipe sequence?

  const eventBeginsSwipe = cast[wheelSequenceAxisKey] === null; // Was this specific event more vertical or more horizontal?

  const eventAxis = Math.abs(deltaY) > Math.abs(deltaX) ? "vertical" : "horizontal";
  ...
  cast[wheelDistanceKey] -= vertical ? deltaY : deltaX; // Update the travel fraction of the component being navigated.

  const targetDimension = vertical ? cast[swipeTarget].offsetHeight : cast[swipeTarget].offsetWidth;
  let fraction = targetDimension > 0 ? cast[wheelDistanceKey] / targetDimension : 0;
  fraction = Math.sign(fraction) * Math.min(Math.abs(fraction), 1);
  const swipeFraction = Math.max(Math.min(fraction, swipeFractionMax), swipeFractionMin); // If the user has dragged enough to reach the previous/next item, then
...
function performImmediateGesture(element, gesture) {
  ...
  }, POST_GESTURE_TIME); // We've handled a gesture, so reset notion of what gestures are in progress.
