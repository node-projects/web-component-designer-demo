...
import { defaultState, ids, raiseChangeEvents, render, rendered, state, template } from "./internal.js";
import PopupButton from "./PopupButton.js"; // A reference to the most recently opened tooltip source.
...
class TooltipButton extends Base {
  connectedCallback() {
    super.connectedCallback(); // Handle edge case where component is opened, removed, then added back.
  ...
  [render](changed) {
    super[render](changed); // Open tooltip on focus/mouseenter, close on mouseleave.
  ...
  get [template]() {
    const result = super[template]; // visually-hidden class from
...
function listenIfOpenAndConnected(element) {
  if (element[state].opened && element.isConnected) {
    ...
    document.removeEventListener("keydown", element[documentKeydownListenerKey]);
