...
import { defaultState, firstRender, ids, keydown, raiseChangeEvents, render, state, template } from "./internal.js";
...
const Base = DelegateFocusMixin(KeyboardMixin(PopupDragSelectMixin(PopupSource)));
/**
...
class PopupButton extends Base {
  ...
  changed) {
    ...
    if (this[firstRender]) {
      ...
      this[ids].source.addEventListener("focus", async event => {
        const popupFocused = ownEvent(this[ids].popup, event); // It's possible to get a focus event in the initial mousedown on the
    ...
    if (changed.sourcePartType) {
      ...
      source.addEventListener("mousedown", event => {
        ...
        event.stopPropagation(); // We don't prevent the default behavior for mousedown. Among other
  ...
  get [template]() {
    const result = super[template]; // When popup is open, it will have focus; don't show focus ring on host.
...
event) {
  ...
  /** @type {any} */
  const element = this; // What has the focus now?
