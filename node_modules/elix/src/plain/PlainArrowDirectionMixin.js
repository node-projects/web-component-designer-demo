import { defaultState, ids, render, state, template } from "../base/internal.js";
...
export default function PlainArrowDirectionMixin(Base) {
  return class PlainArrowDirection extends Base {
    ...
    /** @type {ChangedFlags} */
    changed) {
      super[render](changed); // Rotate the default icons for vertical orientation, flip the default
      ...
      if (changed.orientation || changed.rightToLeft) {
        ...
        const transform = vertical ? "rotate(90deg)" : rightToLeft ? "rotateZ(180deg)" : "";
      ...
      if (changed.dark) {
        ...
        if ("dark" in arrowButtonPrevious) {
          /** @type {any} */
          arrowButtonPrevious.dark = dark;
        }

        if ("dark" in arrowButtonNext) {
          /** @type {any} */
          arrowButtonNext.dark = dark;
        }
      }
    }

    get [template]() {
      const result = super[template]; // Insert our icons into the button slots.

      const arrowButtonPrevious = result.content.querySelector('slot[name="arrowButtonPrevious"]');

      if (arrowButtonPrevious) {
        arrowButtonPrevious.append(fragmentFrom.html`
            <svg
      ...
      const arrowButtonNext = result.content.querySelector('slot[name="arrowButtonNext"]');

      if (arrowButtonNext) {
        arrowButtonNext.append(fragmentFrom.html`
            <svg
              id="arrowIconNext"
