...
import { defaultState, render, setState, state, template } from "../base/internal.js";
...
class PlainCenteredStripOpacity extends CenteredStrip {
  ...
  changed) {
    ...
    if (changed.currentIndex || changed.enableEffects || changed.items || changed.rightToLeft || changed.swipeFraction || changed.transitionDuration) {
      ...
      if (items) {
        ...
        const opacityTransitionValue = showTransition ? `${transitionDuration / 1000}s linear` : null;
        items.forEach((item, index) => {
          const existingTransition = getComputedStyle(item).transition;
          const transition = mergeSinglePropertyTransition(existingTransition, "opacity", opacityTransitionValue);
...
function mergeSinglePropertyTransition(transition, name, value) {
  ...
  if (match) {
    ...
    return transition.substr(0, start) + definition + transition.substr(start + length);
  } else {
