...
class PlainDrawerWithGrip extends PlainDrawerMixin(PlainModalOverlayMixin(DrawerWithGrip)) {
  [render](
  /** @type {ChangedFlags} */
  changed) {
    super[render](changed); // Rotate the default grip icon to reflect the swipe axis.

    if (changed.swipeAxis && this[ids].gripIcon) {
      const transform = this[state].swipeAxis === "horizontal" ? "rotate(90deg)" : "";
      this[ids].gripIcon.style.transform = transform;
    }
  }

  get [template]() {
    const result = super[template]; // Fill the grip slot with our icon.
    // Default grip icon from Material Design icons "drag handle".

    const gripSlot = result.content.querySelector('slot[name="grip"]');

    if (gripSlot) {
      gripSlot.append(fragmentFrom.html`
          <svg
            id="gripIcon"
