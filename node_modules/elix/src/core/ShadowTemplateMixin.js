import { delegatesFocus, hasDynamicTemplate, ids, render, shadowRoot, shadowRootMode, template } from "./internal.js"; // A cache of processed templates, indexed by element class.

const classTemplateMap = new Map(); // A Proxy that maps shadow element IDs to shadow elements.
// This will be return as the element's `this[ids]` property;
// see comments in that property below.

/** @type {any} */

const shadowIdProxyKey = Symbol("shadowIdProxy"); // A reference stored on the shadow element proxy target to get to the actual
...
const proxyElementKey = Symbol("proxyElement"); // A handler used for the shadow element ID proxy.

const shadowIdProxyHandler = {
  get(target, id) {
    // From this proxy, obtain a reference to the original component.
    const element = target[proxyElementKey]; // Get a reference to the component's open or closed shadow root.

    const root = element[shadowRoot]; // Look for a shadow element with the indicated ID.
    ...
    changed) {
      ...
      if (!this[shadowRoot]) {
        ...
        if (template) {
          ...
          root.append(clone); // After this call, we won't be able to rely on being able to access
...
function getTemplate(element) {
  let t = element[hasDynamicTemplate] ? undefined // Always retrieve template
  : classTemplateMap.get(element.constructor); // See if we've cached it

  if (t === undefined) {
    // Ask the component for its template.
    t = element[template]; // A component using this mixin isn't required to supply a template --
