...
export function booleanAttributeValue(name, value) {
  console.warn(`booleanAttributes has moved from dom.js to AttributeMarshallingMixin.js. Please update your imports.`);
...
export function closestFocusableNode(node) {
  for (const current of selfAndComposedAncestors(node)) {
    ...
    const target = current[focusTarget] || current; // We want an element that has a tabIndex of 0 or more. We ignore disabled
...
export function firstFocusableElement(root) {
  ...
  const focusableQuery = 'a[href],area[href],button:not([disabled]),details,iframe,input:not([disabled]),select:not([disabled]),textarea:not([disabled]),[contentEditable="true"],[tabindex]'; // Walk the tree looking for nodes that match the above selectors.

  const walker = walkComposedTree(root,
  /** @type {Node} */
  node => node instanceof HTMLElement && node.matches(focusableQuery) && node.tabIndex >= 0); // We only actually need the first matching value.

  const {
    value
  } = walker.next(); // value, if defined, will always be an HTMLElement, but we do the following
...
export function forwardFocus(origin, target) {
  ...
  if (target) {
    ...
    event => {
      ...
      const desiredTarget = target[focusTarget] || target; // What ancestor can actually take the focus?
...
export function indexOfItemContainingTarget(items, target) {
  return Array.prototype.findIndex.call(items,
  /** @type Node */
...
function* walkComposedTree(node, filter) {
  ...
  if (node instanceof HTMLElement && node.shadowRoot) {
    ...
    children = assignedNodes.length > 0 ? // Walk light DOM nodes assigned to this slot.
    assignedNodes : // Walk light DOM children.
    node.childNodes;
  }
