import { defaultState, firstRender, raiseChangeEvents, render, renderChanges, rendered, rendering, setState, state, stateEffects } from "./internal.js";
/** @type {any} */

const stateKey = Symbol("state");
/** @type {any} */

const raiseChangeEventsInNextRenderKey = Symbol("raiseChangeEventsInNextRender"); // Tracks total set of changes made to elements since their last render.
...
export default function ReactiveMixin(Base) {
  class Reactive extends Base {
    constructor() {
      super(); // Components can inspect `firstRender` during rendering to do special
      ...
      this[firstRender] = undefined; // We want to support the standard HTML pattern of only raising events in
      ...
      this[raiseChangeEvents] = false; // Maintain a change log of all fields which have changed since the
      // component was last rendered.

      this[changedSinceLastRenderKey] = null; // Set the initial state from the default state defined by the component
    ...
    [renderChanges]() {
      ...
      const changed = this[changedSinceLastRenderKey]; // We only render if this is the first render, or state has changed since
      // the last render.
      ...
      if (this[firstRender] || changed) {
        ...
        this[raiseChangeEvents] = this[raiseChangeEventsInNextRenderKey]; // From this point on, we'll assume we won't need to raise events in the
        ...
        this[raiseChangeEventsInNextRenderKey] = false; // We set a flag to indicate that rendering is happening. The component
        // may use this to avoid triggering other updates during the render.

        this[rendering] = true; // Invoke any internal render implementations.

        this[render](changed);
        this[rendering] = false; // Since we've now rendered all changes, clear the change log. If other
        // async render calls are queued up behind this call, they'll see an
        // empty change log, and so skip unnecessary render work.

        this[changedSinceLastRenderKey] = null; // Let the component know it was rendered.

        this[rendered](changed); // We've now rendered for the first time.

        this[firstRender] = false; // Restore state of event flag.
    ...
    async [setState](changes) {
      ...
      const {
        state,
        changed
      } = copyStateWithChanges(this, changes); // We only need to apply the changes to the component state if: a) the
      ...
      Object.freeze(state); // Set this as the component's new state.

      this[stateKey] = state; // If setState was called with the raiseChangeEvents flag set, record that
      ...
      const willRender = this[firstRender] === undefined || this[changedSinceLastRenderKey] !== null; // Add this round of changed fields to the complete log of fields that
      // have changed since the component was last rendered.

      this[changedSinceLastRenderKey] = Object.assign(this[changedSinceLastRenderKey] || {}, changed); // We only need to queue a render if we're in the document and a render
      ...
      if (needsRender) {
        ...
        await Promise.resolve(); // Now that the above promise has resolved, render the component. By the
...
export function copyStateWithChanges(element, changes) {
  ...
  const changed = {}; // Take the supplied changes as the first round of effects.

  let effects = changes; // Loop until there are no effects to apply.
  ...
  while (true) {
    ...
    Object.assign(changed, changedByEffects); // Ask the component if there are any second- (or third-, etc.) order
