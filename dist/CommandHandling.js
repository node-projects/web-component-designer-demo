...
export class CommandHandling {
  ...
  handleCommand(buttons, target) {
    buttons.forEach(b => {
      ...
      if (command === 'new') b.disabled = false;else if (command === 'newFixedWidth') b.disabled = false;else if (command === 'github') b.disabled = false;else b.disabled = !target ? true : !target.canExecuteCommand({
        type: command
      });
    });
  }
